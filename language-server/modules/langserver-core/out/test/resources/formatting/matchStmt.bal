...
function typeGuard1([string, int]|Foo|error|[int, boolean]|int|float x) returns string {
    match x {
        ...
        var {var2} if var2 is Foo => {return "Matched with record int : " + io:sprintf("%s", var2) + " with " + io:sprintf("%s", var2.toString() + "12");}
        var {var1, var2} => {return "Matched with record with ClosedBar1 : "+ io:sprintf("%s", var1) + " with " + io:sprintf("%s" , var2);}
