...
/**
 * Repeat the given text number of times.
 *
 * @param {String} tab ast string to be repeated
 * @param {number} l number of times to be repeated
 * @return {String} generated string
 * */
function repeat(tab, l){
    var string = '';
    for(var i = 0; i <= l; i++){
        string += tab;
    }
 ...
 *
 * @param {Node} node node to be formatted
 * @param {string} parentKind parent kind
 * @return {String} formatted source
 * */
function modifyNode(node, parentKind) {
    var kind = node.kind;
    ...
    if (kind === 'Transaction') {
        if (node && node.condition && node.condition.value) {
            var retry = (node.failedBody ? node.failedBody.statements.filter(isRetry)[0] : null) ||
    ...
    if (node.kind === 'VariableDef' && node.variable &&
        node.variable.typeNode && node.variable.typeNode.kind === 'EndpointType') {
...
/**
 * Get the source of given node.
 *
 * @param {Node} nodeValue  ast to be formatted and source gen
 * @param {boolean} pretty  is pretty
 * @param {number} l  level counter
 * @param {boolean} replaceLambda is lambda to be replaced
 * @return {String} formatted source
 * */
function getSourceOf(nodeValue, pretty, l, replaceLambda) {
    var node = nodeValue;
    if (!node) {
        return '';
    }
    ...
    var i = 0;
    var ws = node.ws ? node.ws.map(function(wsObj){
        return wsObj.ws;
    }) : [];
    var shouldIndent = pretty || !ws.length;
    ...
    switch (node.kind) {
        ...
        case 'ArrayType':
            return getSourceOf(node.elementType, pretty, l, replaceLambda) +
                times(node.dimensions, function(){ return w() + '[' + w() + ']';});
        // auto gen start
...
/**
 * Traverse given AST and modify nodes.
 *
 * @param {Node} json    ast to be formatted.
 * @param {Node} parent  parent node
 * @param {string} parentKind  kind of the parent node
 * @return {Node}  formatted source.
 * */
function build(json, parent, parentKind) {
    var childName;
    ...
    for (childName in json) {
        ...
        if (childName !== 'position' && childName !== 'ws') {
            ...
            if ( child instanceof Object && child.kind) {
                // print(JSON.stringify(child));
                json[childName] = build(child, json, kind);
                ...
                for (var i = 0; i < child.length; i++) {
                    ...
                    if (kind === 'CompilationUnit' && childItem.kind === 'Function' && childItem.lambda) {
                        child.splice(i, 1);
                        i--;
                    } else if (childItem instanceof Object && childItem.kind) {
                        child[i] = build(childItem, json, kind);
                    }
                }
            }
        }
...
/**
 * Generate formatted source for the given AST.
 *
 * @param {String} ast ast to be formatted.
 * @return {String} formatted source.
 * */
function gen(ast){
    var node = JSON.parse(ast).model;
    build(node);
    return getSourceOf(node, true, 0, false);
}
 ...
 * @return {string}
 */
join = function (arr, pretty, replaceLambda, l, wsFunc, defaultWS, separator, suffixLast) {
    var str = '';
