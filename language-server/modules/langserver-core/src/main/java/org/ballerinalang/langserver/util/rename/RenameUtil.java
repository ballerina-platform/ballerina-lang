...
public class RenameUtil {
    ...
    private static Map<String, List<TextEdit>> getChanges(
        ...
        if (QNameReferenceUtil.onModulePrefix(context, nodeAtCursor)) {
            return handleQNameReferenceRename(context, document.get(), nodeAtCursor, newName, honorsChangeAnnotations);
        }
        if (onImportPrefixNode(context, nodeAtCursor)) {
            return handleImportPrefixRename(context, document.get(), nodeAtCursor, newName, honorsChangeAnnotations);
        }
        ...
        for (Map.Entry<Module, List<Location>> entry : locationMap.entrySet()) {
            ...
            for (Location location : locations) {
                ...
                if (honorsChangeAnnotations && CommonUtil.isKeyword(newName)) {
                    String escapedNewName = CommonUtil.escapeReservedKeyword(newName);
                    textEdits.add(new AnnotatedTextEdit(ReferencesUtil.getRange(location),
     ...
     * @param honorsChangeAnnotations Client support for Change Annotations.
     * @return {@link WorkspaceEdit} Workspace edit of changes.
     */
    public static WorkspaceEdit rename(RenameContext context, String newName, boolean honorsChangeAnnotations) {
        Map<String, ChangeAnnotation> changeAnnotationMap = new HashMap<>();
        WorkspaceEdit workspaceEdit = new WorkspaceEdit();
        Map<String, List<TextEdit>> changes = getChanges(context, newName, honorsChangeAnnotations);
    ...
    private static Map<String, List<TextEdit>> handleQNameReferenceRename(ReferencesContext context,
        ...
        return handleImportDeclarationRename(context, document, importDeclarationNode.get(),
                newName, honorsChangeAnnotations);
    }
    ...
    private enum RenameChangeAnnotation {
        ...
        private ChangeAnnotation changeAnnotation;
        private String id;
        ...
        RenameChangeAnnotation(String label, String description, Boolean needsConfirmation, String id) {
            ...
            changeAnnotation.setLabel(label);
            changeAnnotation.setDescription(description);
