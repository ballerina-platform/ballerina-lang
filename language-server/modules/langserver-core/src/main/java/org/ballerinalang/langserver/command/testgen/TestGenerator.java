...
public class TestGenerator {
    ...
    public static class TestFunctionGenerator {
        ...
        private final String[][] valueSpace;
        ...
        public TestFunctionGenerator(BiConsumer<String, String> importsConsumer, PackageID currentPkgId,
            ...
            this(importsConsumer, currentPkgId, function.name.value, function.requiredParams, function.returnTypeNode);
        }

        public TestFunctionGenerator(BiConsumer<String, String> importsConsumer, PackageID currentPkgId,
                                     BLangFunctionTypeNode type) {
            this(importsConsumer, currentPkgId, "", type.params, type.returnTypeNode);
        }
        ...
        private TestFunctionGenerator(BiConsumer<String, String> importsConsumer, PackageID currentPkgId,
                                      String functionName,
                                      List<BLangVariable> params, BLangType returnTypeNode) {
            ...
            // Populate target function's parameters
            for (int i = 0; i < params.size(); i++) {
                BLangVariable variable = params.get(i);
                String paramType = generateTypeDefinition(importsConsumer, currentPkgId,
                                                          variable.typeNode);
                ...
                String[] pValueSpace = getValueSpaceByNode(importsConsumer, currentPkgId, variable.typeNode,
                                                           createTemplateArray(VALUE_SPACE_LENGTH));
                for (int j = 0; j < pValueSpace.length; j++) {
                    // Need to apply transpose of `pValueSpace`
                    // i.e. valueSpace = (pValueSpace)^T
