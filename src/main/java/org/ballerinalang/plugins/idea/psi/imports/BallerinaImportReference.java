...
public class BallerinaImportReference extends FileReference {

    public BallerinaImportReference(@NotNull FileReferenceSet fileReferenceSet, TextRange range, int index,
                                    String text) {
    ...
    protected Object createLookupItem(PsiElement candidate) {
        if (candidate instanceof PsiDirectory) {
            return BallerinaCompletionUtils.createDirectoryLookupElement((PsiDirectory) candidate);
        }
    ...
    protected ResolveResult[] innerResolve(boolean caseSensitive, @NotNull PsiFile file) {
        if (isFirst()) {
            if (".".equals(getCanonicalText())) {
                ...
                return directory != null ? new PsiElementResolveResult[]{new PsiElementResolveResult(directory)} :
                        ResolveResult.EMPTY_ARRAY;
            } else if ("..".equals(getCanonicalText())) {
                PsiDirectory directory = getDirectory();
                PsiDirectory grandParent = directory != null ? directory.getParentDirectory() : null;
                return grandParent != null ? new PsiElementResolveResult[]{new PsiElementResolveResult(grandParent)}
                : ResolveResult.EMPTY_ARRAY;
            }
        }
    ...
    public LocalQuickFix[] getQuickFixes() {
        ...
        //        if (fileReferenceSet instanceof GoImportReferenceSet && !((GoImportReferenceSet)fileReferenceSet)
        // .isRelativeImport()
