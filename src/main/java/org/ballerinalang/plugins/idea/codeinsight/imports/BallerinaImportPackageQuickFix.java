...
public class BallerinaImportPackageQuickFix extends LocalQuickFixAndIntentionActionOnPsiElement
    ...
    public BallerinaImportPackageQuickFix(@NotNull PsiElement element, @NotNull String importPath) {
        super(element);
        myPackageName = "";
        myPackagesToImport = Collections.singletonList(importPath);
    }

    public BallerinaImportPackageQuickFix(@NotNull PsiReference reference) {
    ...
    private static String getText(@NotNull Collection<String> packagesToImport) {
        return getFirstItem(packagesToImport, "") + "? " +
                (packagesToImport.size() > 1 ? "(multiple choices...) " : "");
    }
    ...
    private static boolean notQualified(@Nullable PsiElement startElement) {
        ...
        return false;
        //        return startElement instanceof GoReferenceExpression && ((GoReferenceExpression) startElement)
        // .getQualifier()
        //                == null ||
        //                startElement instanceof GoTypeReferenceExpression && ((GoTypeReferenceExpression)
        // startElement)
        //                        .getQualifier() == null;
    }
    ...
    //    @NotNull
    //    public static List<String> getImportPathVariantsToImport(@NotNull String packageName, @NotNull PsiElement
    // context) {
    //        PsiFile contextFile = context.getContainingFile();
    //
    //        Set<String> imported = contextFile instanceof BallerinaFile
    //                ? ((BallerinaFile) contextFile).getImportedPackagesMap().keySet()
    ...
    //        Collection<GoFile> packages = StubIndex.getElements(GoPackagesIndex.KEY, packageName, project,
    // scope, GoFile
    //                .class);
    ...
    public boolean doAutoImportOrShowHint(@NotNull Editor editor, boolean showHint) {
        ...
        if (packagesToImport.size() == 1) {
            if (BallerinaCodeInsightSettings.getInstance().isAddUnambiguousImportsOnTheFly() && !LaterInvocator
                    .isInModalContext() &&
                    (ApplicationManager.getApplication().isUnitTestMode() || DaemonListeners.canChangeFileSilently
                            (file))) {
        ...
        if (showHint) {
            ...
            if (!BallerinaCodeInsightSettings.getInstance().isShowImportPopup()) return false;
    ...
    private void perform(@NotNull List<String> packagesToImport, @NotNull PsiFile file, @Nullable Editor editor) {
        LOG.assertTrue(editor != null || packagesToImport.size() == 1, "Cannot invoke fix with ambiguous imports on " +
                "null editor");
        if (packagesToImport.size() > 1 && editor != null) {
            ...
            PopupChooserBuilder builder = JBPopupFactory.getInstance().createListPopupBuilder(list).setRequestFocus
                    (true)
            ...
            throw new IncorrectOperationException("Cannot invoke fix with ambiguous imports on editor ()" + editor +
                    ". Packages: " + packages);
        }
    }

    private void perform(@NotNull PsiFile file, @Nullable String pathToImport) {
        if (file instanceof BallerinaFile && pathToImport != null) {
...
//                        }
                        ...
                        BallerinaPsiImplUtil.addImport(file, pathToImport);

                        //                        if (((BallerinaFile) file).getImportedPackagesMap().containsKey
                        // (pathToImport)) {
                        //                            return;
                        //                        }
                        //                        ((BallerinaFile) file).addImport(pathToImport, null);
                    }), "Add import", null);
        }
    }
