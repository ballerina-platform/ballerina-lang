...
public class BallerinaCompletionContributor extends CompletionContributor {
    ...
    //     */
    //    private void addSuggestions(@NotNull CompletionParameters parameters, @NotNull CompletionResultSet
    // resultSet) {
    //
    //        PsiElement element = parameters.getPosition();
    //        PsiElement parent = element.getParent();
    ...
    //        //        } else if (parentPrevSibling instanceof ImportDeclarationNode
    //        //                || parentPrevSibling instanceof PackageDeclarationNode) {
    //        //            // If the previous sibling of the parent is PackageDeclarationNode, that means we have
    // already
    //        // added
    ...
    //                List<WorkerDeclarationNode> workerDeclarations = BallerinaPsiImplUtil.getWorkerDeclarations
    // (parent);
    ...
    //        }
    //        PsiElement superParent = parent.getParent();
    //        if (superParent instanceof AnnotationAttachmentNode && handleAnnotationAttachmentNode(parameters,
    // resultSet)) {
    //            return;
    //        }
    //
    ...
    //    }
    //
    //    private boolean handleBodyNode(@NotNull CompletionParameters parameters, @NotNull CompletionResultSet
    // resultSet,
    ...
    //                    } else if (elementType == BallerinaTypes.COLON) {
    //                        // This will be called instead of handle statement node if there are codes after the
    // caret.
    ...
    //                                PsiElement packageNode = originalFile.findElementAt(prevElement.getTextOffset()
    // - 2);
    //                                suggestElementsFromAPackage(parameters, resultSet, packageNode, true, true,
    // true, true);
    //                            } else {
    ...
    //                            PsiElement packageNode = originalFile.findElementAt(prevElement.getTextOffset() - 2);
    //                            suggestElementsFromAPackage(parameters, resultSet, packageNode, true, true, true,
    // false);
    //                        }
    ...
    //                                prevElement.getTextOffset());
    //                        if (previousNonEmptyElement instanceof LeafPsiElement &&
    //                                ((LeafPsiElement) previousNonEmptyElement).getElementType() == BallerinaTypes
    // .RBRACK) {
    //                            if (resolveElementAndSuggestArrayLength(parameters, resultSet,
    // previousNonEmptyElement)) {
    //                                return;
    //                            }
    //                            return;
    //                        }
    ...
    //                        MapStructLiteralNode node = PsiTreeUtil.getParentOfType(element, MapStructLiteralNode
    // .class);
    //                        if (node != null) {
    ...
    //                        addVariableTypesAsLookups(resultSet, originalFile, element);
    //                    } else {
    //                        ForkJoinStatementNode node = PsiTreeUtil.getParentOfType(element, ForkJoinStatementNode
    // .class);
    //                        if (node != null) {
    //                            PsiElement previousNonEmptyElement = getPreviousNonEmptyElement(originalFile,
    ...
    //            }
    //        } else if (elementType == BallerinaTypes.DOT) {
    //            PsiElement previousNonEmptyElement = getPreviousNonEmptyElement(originalFile, prevElement
    // .getTextOffset());
    //            if (previousNonEmptyElement instanceof LeafPsiElement &&
    //                    ((LeafPsiElement) previousNonEmptyElement).getElementType() == BallerinaTypes.RBRACK) {
    //                if (resolveElementAndSuggestArrayLength(parameters, resultSet, previousNonEmptyElement)) {
    ...
    //                    // Cannot use a switch statement since the types are not constants and declaring them final
    // does
    ...
    //        PsiElement parent = element.getParent();
    //        PsiElement superParent = parent.getParent();
    //        PsiElement nextSibling = PsiTreeUtil.skipSiblingsForward(superParent, PsiWhiteSpace.class, PsiComment
    // .class,
    //                AnnotationAttachmentNode.class);
    //        String type = null;
    //        if (nextSibling == null) {
    ...
    //                || nextSibling.getParent() instanceof ActionDefinitionNode || parent instanceof
    ...
    //                                             @NotNull CompletionResultSet resultSet, @NotNull PsiElement
    // prevElement) {
    //        PsiFile originalFile = parameters.getOriginalFile();
    //        PsiElement token = getPreviousNonEmptyElement(originalFile, prevElement.getTextOffset());
    //        if (token != null && token instanceof LeafPsiElement) {
    ...
    //
    //    /**
    //     * Contains logic to add lookup elements for a parameter node if the previous node is not an identifier or
    // leaf
    //     * element node.
    //     *
    //     * @param parameters parameters which passed to completion contributor
    ...
    //            IdentifierPSINode packageName = PsiTreeUtil.findElementOfClassAtOffset(prevSibling
    // .getContainingFile(),
    //                    prevSibling.getTextOffset() + prevSibling.getTextLength() - 2, IdentifierPSINode.class,
    // false);
    //            if (packageName == null) {
    //                return;
    //            }
    ...
    //                                    addStructFields(parameters, resultSet, null, structDefinitionNode, true,
    // true);
    //                                }
    //                            } else {
    //                                addLookups(resultSet, originalFile, true, true, true, false);
    ...
    //    }
    //
    //    private boolean addStructFields(@NotNull CompletionParameters parameters, @NotNull CompletionResultSet
    // resultSet,
    //                                    PsiElement element) {
    //        // If we are in the {} of a struct variable reference node, we need to show fields.
    ...
    //            IElementType elementType = ((LeafPsiElement) token).getElementType();
    //            if (elementType == BallerinaTypes.COLON) {
    //                // This will be called if there are no lines after the caret. If there are codes after the
    // caret, it
    ...
    //            return true;
    //        }
    //        // Resolved element can be either a VariableDefinitionNode or a ParameterNode or a
    // GlobalVariableDefinitionNode.
    //        // This happens if the reference is StatementReference.
    //        // If the reference is a NameReference, resolved element will be an identifier node. So the definition
    // node
    //        // is the parent node.
    ...
    //     * @param parameters                 parameters which contain details of completion invocation
    //     * @param resultSet                  result list which is used to add lookups
    //     * @param prevElement                element which can be used to resolve and get the struct variable
    // definition
    ...
    //        //                continue;
    //        //            }
    //        //            // Resolved element will be an identifier. So we need to get the parent which is a
    // connector or
    //        //            // native connector.
    ...
    //        AnnotationAttachmentNode attachmentNode = PsiTreeUtil.getParentOfType(element, AnnotationAttachmentNode
    // .class);
    //        if (attachmentNode == null) {
    //            return;
    //        }
    //        AnnotationDefinitionNode definitionNode = BallerinaPsiImplUtil.getAnnotationDefinitionNode
    // (attachmentNode);
