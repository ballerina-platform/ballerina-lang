...
public class BirVariableOptimizer extends BIRVisitor {
    ...
    private boolean isDeadInstruction(List<BIRNode.BIRVariableDcl> lhsOpOfRemovedInstructions,
        ...
        if (lhsOp == null || isInValidInstruction(instruction.getKind())) {
            return false;
        }
    ...
    private boolean isInValidInstruction(InstructionKind kind) {
        return kind == InstructionKind.TYPE_CAST || kind == InstructionKind.DIV ||
                kind == InstructionKind.NEW_STRUCTURE;
    }
