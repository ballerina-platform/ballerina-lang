...
import static org.wso2.ballerinalang.compiler.bir.codegen.JvmConstants.BXML_TYPE_IMPL;
...
import static org.wso2.ballerinalang.compiler.bir.codegen.JvmConstants.TYPES;
...
class JvmTypeGen {
    ...
    private static void createRecordField(MethodVisitor mv, BField field) {
        ...
        mv.visitTypeInsn(NEW, BFIELD_IMPL);
        mv.visitInsn(DUP);
        ...
        mv.visitMethodInsn(INVOKESPECIAL, BFIELD_IMPL, JVM_INIT_METHOD, String.format("(L%s;L%s;I)V", TYPE,
                                                                                      STRING_VALUE), false);
    }
    ...
    static void loadType(MethodVisitor mv, BType bType) {
        ...
        mv.visitFieldInsn(GETSTATIC, TYPES, typeFieldName, String.format("L%s;", TYPE));
    }
    ...
    private static void loadXmlType(MethodVisitor mv, BXMLType bType) {
        ...
        mv.visitTypeInsn(NEW, BXML_TYPE_IMPL);
        mv.visitInsn(DUP);
        ...
        // invoke the constructor
        mv.visitMethodInsn(INVOKESPECIAL, BXML_TYPE_IMPL, JVM_INIT_METHOD, String.format("(L%s;Z)V", TYPE), false);
    }
    ...
    private static void loadErrorType(MethodVisitor mv, BErrorType errorType) {
        ...
        if (packageID.orgName.value.equals(BALLERINA) && packageID.name.value.equals(BUILT_IN_PACKAGE_NAME)) {
            mv.visitFieldInsn(GETSTATIC, TYPES, TYPES_ERROR, String.format("L%s;", ERROR_TYPE));
            return;
        }
        String typeOwner =
