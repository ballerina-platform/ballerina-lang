...
public class ParameterDesugar extends BLangNodeVisitor {
    ...
    private void updateFunctionParams(BLangFunction funcNode, BLangSimpleVarRef varRefExpr) {
        ...
        BVarSymbol varSymbol = ASTBuilderUtil.duplicateParamSymbol((BVarSymbol) varRefExpr.symbol, funcSymbol);
        varSymbol.flags = 0;
        funcSymbol.scope.define(varSymbol.name, varSymbol);
        varRefExpr.symbol = varSymbol;
        funcSymbol.params.add(varSymbol);
        funcType.paramTypes.add(varSymbol.type);
        funcNode.requiredParams.add(ASTBuilderUtil.createVariable(varSymbol.pos, varSymbol.name.value, varSymbol.type,
                                                                 null, varSymbol));
    }
