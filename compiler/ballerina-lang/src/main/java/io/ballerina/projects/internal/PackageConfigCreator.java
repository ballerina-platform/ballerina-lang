...
public class PackageConfigCreator {
    ...
    private static PackageConfig createPackageConfig(PackageData packageData,
        ...
        if (!packageData.resources().isEmpty()) {
            resources = getResourceConfigs(packageData.resources());
        }
        if (!packageData.testResources().isEmpty()) {
            testResources = getResourceConfigs(packageData.testResources());
        }
    ...
    private static List<ResourceConfig> getResourceConfigs(List<Path> resources) {
        // TODO: no need Remove duplicate paths before processing
        Set<Path> distinctResources = new HashSet<>(resources);
        return distinctResources.stream().map(PackageConfigCreator::createResourceConfig).collect(Collectors.toList());
    }

    private static ResourceConfig createResourceConfig(Path path) {
        final DocumentId documentId = DocumentId.create(path.toString(), null);
        return ProvidedResourceConfig.from(documentId, path);
    }
