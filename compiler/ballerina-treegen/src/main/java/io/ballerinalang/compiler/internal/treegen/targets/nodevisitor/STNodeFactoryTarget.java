...
import static io.ballerinalang.compiler.internal.treegen.TreeGenConfig.INTERNAL_NODE_FACTORY_TEMPLATE_KEY;
...
/**
 * Generates a node factory java class for the internal syntax tree.
 *
 * @since 1.3.0
 */
public class STNodeFactoryTarget extends AbstractNodeVisitorTarget {

    public STNodeFactoryTarget(TreeGenConfig config) {
        super(config);
    }
    ...
    protected String getClassName() {
        return INTERNAL_NODE_FACTORY_CN;
    }

    @Override
    protected String getSuperClassName() {
        return INTERNAL_BASE_NODE_FACTORY_CN;
    }

    @Override
    protected List<String> getImportClasses() {
        List<String> imports = new ArrayList<>();
        imports.add(getClassFQN(config.getOrThrow(EXTERNAL_NODE_PACKAGE_KEY), SYNTAX_KIND_CN));
        return imports;
    }

    @Override
    protected String getTemplateName() {
        return config.getOrThrow(INTERNAL_NODE_FACTORY_TEMPLATE_KEY);
    }
}
