...
service hello on new http:Listener(9090) {
    ...
    resource function bindJson(http:Caller caller, http:Request req, json orderDetails) {
        ...
        } else if (!isValid(orderDetails.Details.Name.toString().matches("[a-zA-Z]+"))) {
            res.statusCode = 400;
            res.setPayload("Order Details Name containts invalid data");
        } else {
            // Since there is no validation error, mark the details as trusted data and set it to the response.
    ...
    resource function bindXML(http:Caller caller, http:Request req, xml store) {
        ...
        if (!isValid(city.getTextValue().matches("\\d+"))) {
            ...
            // Since there is no validation error, mark the city as trusted data and set it to the response.
    ...
    resource function bindStruct(http:Caller caller, http:Request req, Student student) {
        ...
        if (!isValid(student.Name.matches("[a-zA-Z]+"))) {
            ...
            // Since there is no validation error, mark the inputs as trusted data and set them to the response.
