...
// `cache` config of the client endpoint. In this example, the `isShared` field of the `cacheConfig` is set to true,
// as the cache will be a public cache in this particular scenario. <br>
...
// the `cacheConfig`. Currently, there are only 2 policies: `CACHE_CONTROL_AND_VALIDATORS` (the default policy) and
// `RFC_7234`.
...
service<http:Service> helloWorld bind {port: 8080} {
    ...
    sayHello(endpoint caller, http:Request req) {
        ...
        // The `ResponseCacheControl` object in the `Response` object can be used for setting the cache control
        // directives associated with the response. In this example, `max-age` directive is set to 15 seconds
        // indicating that the response will be fresh for 15 seconds. The `must-revalidate` directive instructs that
        // the cache should not serve a stale response without validating it with the origin server first.
        // The `public` directive is set by setting `isPrivate=false`. This indicates that the response can be cached
        // even by intermediary caches which serve multiple users.
        ...
        // The `setETag()` function can be used for generating ETags for `string`, `json`, `xml`, and `blob` types.
        // This uses the `getCRC32()` function from the `ballerina/crypto` package for generating the ETag.
        res.setETag(payload);
