...
# This is done by setting the runtime argument: <br> '-B[tracelog.http].level=TRACE'. <br>
$ ballerina run http-trace-logs.bal -B[tracelog.http].level=TRACE
ballerina: deploying service(s) in 'http-trace-logs.bal'
