...
service<http:WebSocketService> SimpleProxyService bind serviceEndpoint {
    ...
    onOpen(endpoint caller) {
        ...
        wsClientEp->ready() but { error e => log:printError("Error calling ready on client", err=e) };
    }
    ...
    onText(endpoint caller, string text, boolean finalFrame) {
        endpoint http:WebSocketClient clientEp = getAssociatedClientEndpoint(caller);
        clientEp -> pushText(text, final = finalFrame) but { error e => log:printError("Error occurred when sending text message", err=e) };
    }
    ...
    onBinary(endpoint caller, blob data, boolean finalFrame) {
        endpoint http:WebSocketClient clientEp = getAssociatedClientEndpoint(caller);
        clientEp -> pushBinary(data, final = finalFrame) but { error e => log:printError("Error occurred when sending binary message", err=e) };
    }
    ...
    onClose(endpoint caller, int statusCode, string reason) {
        endpoint http:WebSocketClient clientEp = getAssociatedClientEndpoint(caller);
        clientEp -> close(statusCode, reason) but { error e => log:printError("Error occurred when closing the connection", err=e) };
...
service<http:WebSocketClientService> ClientService {
    ...
    onText(endpoint caller, string text, boolean finalFrame) {
        endpoint http:WebSocketListener serverEp = getAssociatedServerEndpoint(caller);
        serverEp-> pushText(text, final = finalFrame) but { error e => log:printError("Error occurred when sending text message", err=e) };
    }

    //This resource is triggered when a new binary frame is received from the remote backend.
    onBinary(endpoint caller, blob data, boolean finalFrame) {
        endpoint http:WebSocketListener serverEp = getAssociatedServerEndpoint(caller);
        serverEp-> pushBinary(data, final = finalFrame) but { error e => log:printError("Error occurred when sending binary message", err=e) };
    }

    //This resource is triggered when a client connection is closed by the remote backend.
    onClose(endpoint caller, int statusCode, string reason) {
        endpoint http:WebSocketListener serverEp = getAssociatedServerEndpoint(caller);
        serverEp-> close(statusCode, reason) but { error e => log:printError("Error occurred when closing the connection", err=e) };
        _ = caller.attributes.remove(ASSOCIATED_CONNECTION);
    }
}
