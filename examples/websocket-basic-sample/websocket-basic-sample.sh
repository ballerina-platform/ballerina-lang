...
# To check the the behavior when the server sends a ping message to the browser client use the following command:
$ ws.send("ping");

#To close the connection:
$ ws.close(1000, "I want to go");

# To close connection from the server side:
$ ws.send("closeMe");
