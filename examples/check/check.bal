...
type Person object {
    ...
    function getAddress() returns (Address|error) {
...
function validateAddress(Person person) returns (boolean|error) {
    //Expression "person.getAddress()!city" produces a value which is union type of string|error.
    ...
    string city = check person.getAddress()!city;
...
function validateAddressAgain(Person person) returns boolean {
    //If the expression "person.getAddress()!city" evaluates an error and since enclosing function's return type doesn't have an error as an alternative (only boolean), then Check expression throws the resulted error.
