...
public function main() {
    ...
    map<string> animals = words.entries().map(toUpper);
    io:println(animals);
    ...
    int total = numbers.reduce(totalReducer, 0);
    io:println("Total: " + total.toString());

    int totalWithInitialValue = numbers.reduce(totalReducer, 5);
    io:println("Total with initial value: " + totalWithInitialValue.toString());
    ...
    j.entries().map(function ([string, json] pair) returns string {
            var [key, value] = pair;
...
function totalReducer(int accumulator, int currentValue) returns int {
