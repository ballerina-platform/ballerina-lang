...
service FailoverProxyService on new http:Listener(9090) {
    ...
    resource function onOpen(http:WebSocketCaller caller) {
        ...
        // Defines the webSocket failover client endpoint.
        http:WebSocketFailoverClient wsClientEp = new(
            {callbackService: FailoverClientRetryService,

            // Defines a set of target.
            targetUrls: [ "ws://localhost:9095/failover/retry/ws", "ws://localhost:9096/failover/retry/ws",
            "ws://localhost:9094/failover/retry/ws"],
...
service FailoverClientRetryService = @http:WebSocketServiceConfig {} service {
...
function getAssociatedClientEndpoint(http:WebSocketCaller ep) returns (http:WebSocketFailoverClient) {
    http:WebSocketFailoverClient wsClient = <http:WebSocketFailoverClient>ep.getAttribute(ASSOCIATED_CONNECTION);
    return wsClient;
}
