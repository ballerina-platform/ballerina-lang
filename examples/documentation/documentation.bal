...
documentation {
    ...
    one attached function definition, `doThatOnObject` perform that functionality
    on given `DummyObject` instance.
...
documentation {
   `DummyType` is a user defined record type in Ballerina program.
              This `DummyType` is used to hold a string type data and int type
              data.
