...
http:Client http2serviceClientEP = new("http://localhost:7090", config = { httpVersion: "2.0" });
...
service http11Service on new http:Listener(9090) {
    ...
    resource function http11Resource(http:Caller caller, http:Request clientRequest) {
...
service http2service on http2serviceEP {
    ...
    resource function http2Resource(http:Caller caller, http:Request clientRequest) {
