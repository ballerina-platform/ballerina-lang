...
public function main() {
    ...
    // from `x` to `y` including `x` and `y`, in increasing order.
    ...
    // A range expression `x ..< y` can be used to iterate integers
    // from `x` to `y` including `x` but excluding `y`, in increasing order.
    io:println("\nforeach for 25 ..< 28");
    foreach int i in 25 ..< 28 {
        io:println(i);
    }

    // The result of a range expression can also be used by assigning to
