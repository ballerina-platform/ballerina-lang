...
//of user-defined functions. This allow users to restrict
//passing untrusted (tainted) data into a security sensitive
//parameter.
...
function main (string... args) {
    ...
    //Sensitive parameters of functions built-in to Ballerina
    //are decorated with the @sensitive annotation. This ensures
    //that tainted data cannot be passed into the security
    //sensitive parameter.
    //
    //For example, the taint checking mechanism of Ballerina
    //completely prevents SQL injection vulnerabilities by
    //disallowing tainted data in the SQL query.
    //
    //This line results in a compiler error because the query
    ...
    //This line results in a compiler error because a user-provided
    ...
    if (isInteger(args[0])) {
        ...
        //the proceeding value trusted and pass it to a sensitive
        //parameter.
    ...
    while (dataTable.hasNext()) {
        ...
        //The return values of certain functions built-in to Ballerina
        //are decorated with the @tainted annotation to denote that the
        //return value should be untrusted (tainted). One such example
        //is the data read from a database.
        //
        //This line results in a compiler error because a value derived
        ...
        //This line results in a compiler error because the 'sanitize'
        ...
        //the return value is safe and can be trusted.
...
//the function should be trusted (untainted) even though the
//return value is derived from tainted data.
