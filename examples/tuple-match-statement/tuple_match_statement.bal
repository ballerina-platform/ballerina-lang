...
public function main() {
    ...
    (string, int)|(float, boolean)|(boolean, int)|(int, boolean)|int|float b4 = (455, true);
...
function bar(any b) {
    match b {
        ...
        var (s, i) if (s is string && i is int) => io:println("'s' is string and 'i' is int : " + io:sprintf("%s", b));
        // This pattern check for tuple type of two variables and first variable should be of type 'float'.
        var (s, i) if s is float => io:println("Only 's' is float : " + io:sprintf("%s", b));
        // This pattern check for tuple type of two variables and second variable should be of type 'int'.
        var (s, i) if i is int => io:println("Only 'i' is int : " + io:sprintf("%s", b));
        ...
        var s if s is float => io:println("'s' is float only : " + io:sprintf("%s", b));
    }
}
