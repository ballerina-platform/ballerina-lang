...
public type Participant2pcClientEP client object {
    ...
    public remote function prepare(string transactionId) returns string|error {
        ...
        if (statusCode == http:STATUS_404) {
            error err = error(TRANSACTION_UNKNOWN);
            return err;
        } else if (statusCode == http:STATUS_200) {
    ...
    public remote function notify(string transactionId, string message) returns string|error {
        ...
        } else if ((statusCode == http:STATUS_400 && msg == NOTIFY_RESULT_NOT_PREPARED_STR) ||
            (statusCode == http:STATUS_404 && msg == TRANSACTION_UNKNOWN) ||
            (statusCode == http:STATUS_500 && msg == NOTIFY_RESULT_FAILED_EOT_STR)) {
