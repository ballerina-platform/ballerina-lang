...
public class Register {
    ...
    public static Object register(Environment env, BObject listener, BObject service, Object name) {
        ...
        try {
            final Map<String, MethodTypes> resourceRegistry = getResourceRegistry(service);
    ...
    private static Map<String, MethodTypes> getResourceRegistry(BObject service) {
        Map<String, MethodTypes> registry = new HashMap<>(5);
        final MethodTypes[] attachedFunctions = service.getType().setMethodTypes();
        for (MethodTypes resource : attachedFunctions) {
