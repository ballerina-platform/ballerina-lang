 ...
 * @since 0.94
 */
public class BHttpDispatcher {

    public static BHttpService findService(BHTTPServicesRegistry servicesRegistry, HttpCarbonMessage inboundReqMsg) {
        try {
            Map<String, BHttpService> servicesOnInterface;
            ...
            BHttpService service = servicesOnInterface.get(basePath);
     ...
     */
    public static BHttpResource findResource(BHTTPServicesRegistry servicesRegistry, HttpCarbonMessage inboundMessage) {
        ...
        try {
            ...
            BHttpService service = BHttpDispatcher.findService(servicesRegistry, inboundMessage);
            if (service == null) {
            ...
            // Find the Resource
            return BHttpResourceDispatcher.findResource(service, inboundMessage);
        } catch (Exception e) {
            throw new BallerinaConnectorException(e.getMessage());
        }
    }

    public static BValue[] getSignatureParameters(BHttpResource httpResource, HttpCarbonMessage httpCarbonMessage,
        ...
        BHttpUtil.enrichHttpCallerWithConnectionInfo(httpCaller, httpCarbonMessage, httpResource, endpointConfig);
        BHttpUtil.enrichHttpCallerWithNativeData(httpCaller, httpCarbonMessage, endpointConfig);
        serviceEndpoint.put(SERVICE_ENDPOINT_CONNECTION_FIELD, httpCaller);

        BHttpUtil.populateInboundRequest(inRequest, inRequestEntity, mediaType, httpCarbonMessage, programFile);

        BSignatureParams signatureParams = httpResource.getSignatureParams();
        ...
        try {
            ...
            httpCarbonMessage.setHttpStatusCode(Integer.valueOf(HttpConstants.HTTP_BAD_REQUEST));
    ...
    private static BValue populateAndGetEntityBody(BMap<String, BValue> inRequest,
        ...
        BHttpUtil.populateEntityBody(null, inRequest, inRequestEntity, true, true);
        try {
    ...
    public static boolean shouldDiffer(BHttpResource httpResource) {
        return (httpResource != null && httpResource.getSignatureParams().getEntityBody() != null);
    }

    private BHttpDispatcher() {
    }
}
