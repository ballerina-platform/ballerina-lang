...
public class HttpServerChannelInitializer extends ChannelInitializer<SocketChannel> {
    ...
    private OCSPResp getOcspResponse() throws IOException, KeyStoreException, CertificateVerificationException,
    ...
    public void configureHttpPipeline(ChannelPipeline serverPipeline, String initialHttpScheme) {
        ...
        if (initialHttpScheme.equals(Constants.HTTP_SCHEME)) {
            ...
            if (httpAccessLogEnabled) {
                serverPipeline.addLast(Constants.HTTP_ACCESS_LOG_HANDLER, new HttpAccessLoggingHandler(
                        Constants.ACCESS_LOG));
            }
        }
        ...
        if (httpTraceLogEnabled) {
            pipeline.addLast(Constants.HTTP_TRACE_LOG_HANDLER,
                             new HttpTraceLoggingHandler(
                                     Constants.TRACE_LOG_DOWNSTREAM));
        }
        if (httpAccessLogEnabled) {
            pipeline.addLast(Constants.HTTP_ACCESS_LOG_HANDLER, new HttpAccessLoggingHandler(
                    Constants.ACCESS_LOG));
        }
    ...
    class Http2PipelineConfiguratorForServer extends ApplicationProtocolNegotiationHandler {
        ...
        protected void configurePipeline(ChannelHandlerContext ctx, String protocol) {
            ...
            if (ApplicationProtocolNames.HTTP_2.equals(protocol)) {
                ...
                ctx.pipeline().addLast(Constants.HTTP2_SOURCE_CONNECTION_HANDLER,
                                       new Http2SourceConnectionHandlerBuilder(
