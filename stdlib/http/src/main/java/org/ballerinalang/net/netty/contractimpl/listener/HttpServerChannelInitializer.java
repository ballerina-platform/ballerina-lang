...
import org.ballerinalang.net.netty.contractimpl.listener.http2.Http2SourceConnectionHandlerBuilder;
import org.ballerinalang.net.netty.contractimpl.listener.http2.Http2ToHttpFallbackHandler;
import org.ballerinalang.net.netty.contractimpl.listener.http2.Http2WithPriorKnowledgeHandler;
...
import static org.ballerinalang.net.netty.contractimpl.common.Util.setSslHandshakeTimeOut;
...
public class HttpServerChannelInitializer extends ChannelInitializer<SocketChannel> {
    ...
    private org.ballerinalang.net.netty.contract.config.RequestSizeValidationConfig reqSizeValidationConfig;
        ...
        if (http2Enabled) {
            if (sslHandlerFactory != null) {
                if (ocspStaplingEnabled) {
                    ...
                    serverPipeline
                            .addLast(sslHandler, new Http2PipelineConfiguratorForServer(this, sslHandler.engine()));
                    serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP2_EXCEPTION_HANDLER, new Http2ExceptionHandler());
                }
            } else {
            ...
            if (sslHandlerFactory != null) {
                ...
                configureHttpPipeline(serverPipeline, org.ballerinalang.net.netty.contract.Constants.HTTP_SCHEME);
            }
        }
    }
    ...
    private void configureSslForHttp(ChannelPipeline serverPipeline, SocketChannel ch)
        ...
        if (ocspStaplingEnabled) {
            ...
            setSslHandshakeTimeOut(sslConfig, sslHandler);
            serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.SSL_HANDLER, sslHandler);
            if (validateCertEnabled) {
                serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_CERT_VALIDATION_HANDLER,
                                       new CertificateValidationHandler(sslEngine, cacheDelay, cacheSize));
            }
        }
        serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.SSL_COMPLETION_HANDLER,
    ...
    public void configureHttpPipeline(ChannelPipeline serverPipeline, String initialHttpScheme) {
        ...
        if (initialHttpScheme.equals(org.ballerinalang.net.netty.contract.Constants.HTTP_SCHEME)) {
            serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_ENCODER, new HttpResponseEncoder());
            serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_DECODER,
            ...
            serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_COMPRESSOR, new CustomHttpContentCompressor());
            serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_CHUNK_WRITER, new ChunkedWriteHandler());

            if (httpTraceLogEnabled) {
                serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_TRACE_LOG_HANDLER,
                                       new HttpTraceLoggingHandler(
                        org.ballerinalang.net.netty.contract.Constants.TRACE_LOG_DOWNSTREAM));
            }
            if (httpAccessLogEnabled) {
                serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_ACCESS_LOG_HANDLER, new HttpAccessLoggingHandler(
                        org.ballerinalang.net.netty.contract.Constants.ACCESS_LOG));
            }
        }
        ...
        serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.WEBSOCKET_SERVER_HANDSHAKE_HANDLER,
                               new WebSocketServerHandshakeHandler(this.serverConnectorFuture,
                                                                   webSocketCompressionEnabled));
        serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.BACK_PRESSURE_HANDLER, new BackPressureHandler());
        serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_SOURCE_HANDLER,
        ...
        if (socketIdleTimeout >= 0) {
            serverPipeline.addBefore(org.ballerinalang.net.netty.contract.Constants.HTTP_SOURCE_HANDLER, org.ballerinalang.net.netty.contract.Constants.IDLE_STATE_HANDLER,
                                     new IdleStateHandler(0, 0, socketIdleTimeout, TimeUnit.MILLISECONDS));
        }
        serverPipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_EXCEPTION_HANDLER, new HttpExceptionHandler());
    }
    ...
    private void configureH2cPipeline(ChannelPipeline pipeline) {
        ...
        pipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_SERVER_CODEC, sourceCodec);
        pipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_COMPRESSOR, new CustomHttpContentCompressor());
        if (httpTraceLogEnabled) {
        ...
        if (httpAccessLogEnabled) {
            pipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP_ACCESS_LOG_HANDLER, new HttpAccessLoggingHandler(
                    org.ballerinalang.net.netty.contract.Constants.ACCESS_LOG));
        }
        pipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP2_UPGRADE_HANDLER,
        ...
        pipeline.addLast(org.ballerinalang.net.netty.contract.Constants.HTTP2_TO_HTTP_FALLBACK_HANDLER,
    ...
    class Http2PipelineConfiguratorForServer extends ApplicationProtocolNegotiationHandler {
        ...
        protected void configurePipeline(ChannelHandlerContext ctx, String protocol) {
            ...
            if (ApplicationProtocolNames.HTTP_2.equals(protocol)) {
                ...
                configureHttpPipeline(ctx.pipeline(), org.ballerinalang.net.netty.contract.Constants.HTTP_SCHEME);
