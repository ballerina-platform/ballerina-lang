...
import static org.ballerinalang.net.http.WebSocketConstants.CONNECTION_REFUSED;
...
import static org.ballerinalang.net.http.WebSocketConstants.UNEXPECTED_ERROR;
...
public class WebSocketClientHandshakeListener implements ClientHandshakeListener {
    ...
    public void onError(Throwable throwable, HttpCarbonResponse response) {
        ...
        if (throwable.getMessage().contains(CONNECTION_REFUSED) ||
                throwable.getMessage().contains(UNEXPECTED_ERROR) ||
                throwable.getMessage().contains("Invalid handshake")) {
            if (webSocketClient.getNativeData(IS_CONNECTION_MADE) == null) {
