 ...
 * @since 0.983.1
 */
public class BWebSocketClientHandshakeListener implements ClientHandshakeListener {

    private final Context context;
    private final BWebSocketService wsService;
    private final BWebSocketClientConnectorListener clientConnectorListener;
    private final boolean readyOnConnect;
    private CountDownLatch countDownLatch;

    public BWebSocketClientHandshakeListener(Context context, BWebSocketService wsService,
                                             BWebSocketClientConnectorListener clientConnectorListener,
    ...
    public void onSuccess(WebSocketConnection webSocketConnection, HttpCarbonResponse carbonResponse) {
        ...
        webSocketClientEndpoint.put(WebSocketConstants.CLIENT_RESPONSE_FIELD,
                                    BHttpUtil.createResponseStruct(context, carbonResponse));
        ...
        BWebSocketOpenConnectionInfo connectionInfo = new BWebSocketOpenConnectionInfo(
        ...
        BWebSocketUtil.populateEndpoint(webSocketConnection, webSocketClientEndpoint);
    ...
    public void onError(Throwable throwable, HttpCarbonResponse response) {
        ...
        if (response != null) {
            webSocketClientEndpoint.put(WebSocketConstants.CLIENT_RESPONSE_FIELD,
                                        BHttpUtil.createResponseStruct(context, response));
        }
        BMap<String, BValue> webSocketConnector = BLangConnectorSPIUtil.createObject(
                context, PROTOCOL_PACKAGE_HTTP, WebSocketConstants.WEBSOCKET_CONNECTOR);
        BWebSocketOpenConnectionInfo connectionInfo = new BWebSocketOpenConnectionInfo(
        ...
        webSocketConnector.addNativeData(WebSocketConstants.NATIVE_DATA_WEBSOCKET_CONNECTION_INFO, connectionInfo);
        countDownLatch.countDown();
        BWebSocketDispatcher.dispatchError(connectionInfo, throwable);
    }
}
