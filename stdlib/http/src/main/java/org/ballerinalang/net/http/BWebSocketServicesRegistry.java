...
public class BWebSocketServicesRegistry {
    ...
    public void registerService(BWebSocketService service) {
        ...
        try {
            getUriTemplate().parse(basePath, service, new BWebSocketDataElementFactory());
    ...
    public URITemplate<BWebSocketService, WebSocketMessage> getUriTemplate() throws URITemplateException {
        if (uriTemplate == null) {
            uriTemplate = new URITemplate<>(new Literal<>(new BWebSocketDataElement(), "/"));
        }
        return uriTemplate;
    }
