 ...
 * @since 0.94
 */
public class BWebSocketDispatcher {

    private static final Logger log = LoggerFactory.getLogger(BWebSocketDispatcher.class);

    private BWebSocketDispatcher() {
    }
     ...
     */
    static BWebSocketService findService(BWebSocketServicesRegistry servicesRegistry,
        ...
        try {
            ...
            BWebSocketService service = servicesRegistry.getUriTemplate().matches(requestUri.getPath(), pathParams,
                                                                                 webSocketHandshaker);
            if (service == null) {
    ...
    static void dispatchTextMessage(BWebSocketOpenConnectionInfo connectionInfo,
                                    WebSocketTextMessage textMessage) throws IllegalAccessException {
        WebSocketConnection webSocketConnection = connectionInfo.getWebSocketConnection();
        BWebSocketService wsService = connectionInfo.getService();
        ...
        if (dataTypeTag == TypeTags.STRING_TAG) {
            ...
            Executor.submit(onTextMessageResource, new BWebSocketResourceCallableUnitCallback(webSocketConnection),
                            null, null, bValues);
    ...
    private static void dispatchResourceWithAggregatedData(WebSocketConnection webSocketConnection,
        ...
        try {
            ...
            Executor.submit(onTextMessageResource,
                            new BWebSocketResourceCallableUnitCallback(webSocketConnection),
                            null, null, bValues);
    ...
    static void dispatchBinaryMessage(BWebSocketOpenConnectionInfo connectionInfo,
        ...
        Executor.submit(onBinaryMessageResource, new BWebSocketResourceCallableUnitCallback(webSocketConnection), null,
                        null, bValues);
    }

    static void dispatchControlMessage(BWebSocketOpenConnectionInfo connectionInfo,
        ...
        if (controlMessage.getControlSignal() == WebSocketControlSignal.PING) {
            dispatchPingMessage(connectionInfo, controlMessage);
        } else if (controlMessage.getControlSignal() == WebSocketControlSignal.PONG) {
            dispatchPongMessage(connectionInfo, controlMessage);
        }
    }

    private static void dispatchPingMessage(BWebSocketOpenConnectionInfo connectionInfo,
                                            WebSocketControlMessage controlMessage) throws IllegalAccessException {
        WebSocketConnection webSocketConnection = connectionInfo.getWebSocketConnection();
        BWebSocketService wsService = connectionInfo.getService();
        ...
        Executor.submit(onPingMessageResource, new BWebSocketResourceCallableUnitCallback(webSocketConnection), null,
                        null, bValues);
    }

    private static void dispatchPongMessage(BWebSocketOpenConnectionInfo connectionInfo,
                                            WebSocketControlMessage controlMessage) throws IllegalAccessException {
        WebSocketConnection webSocketConnection = connectionInfo.getWebSocketConnection();
        BWebSocketService wsService = connectionInfo.getService();
        ...
        bValues[0] = connectionInfo.getWebSocketEndpoint();
        bValues[1] = new BValueArray(controlMessage.getByteArray());
        Executor.submit(onPongMessageResource, new BWebSocketResourceCallableUnitCallback(webSocketConnection), null,
                        null, bValues);
    }

    static void dispatchCloseMessage(BWebSocketOpenConnectionInfo connectionInfo,
    ...
    static void dispatchError(BWebSocketOpenConnectionInfo connectionInfo, Throwable throwable) {
        try {
            BWebSocketUtil.setListenerOpenField(connectionInfo);
        } catch (IllegalAccessException e) {
            connectionInfo.getWebSocketEndpoint().put(WebSocketConstants.LISTENER_IS_OPEN_FIELD, new BBoolean(false));
        }
        BWebSocketService webSocketService = connectionInfo.getService();
    ...
    private static BError getError(BWebSocketOpenConnectionInfo connectionInfo, Throwable throwable) {
        ...
        if (context != null) {
            return BHttpUtil.getError(context, errMsg);
        } else {
    ...
    static void dispatchIdleTimeout(BWebSocketOpenConnectionInfo connectionInfo) throws IllegalAccessException {
        WebSocketConnection webSocketConnection = connectionInfo.getWebSocketConnection();
        BWebSocketService wsService = connectionInfo.getService();
