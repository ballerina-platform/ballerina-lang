...
public class SslConfiguration {
    ...
    public void setVerifyClient(String verifyClient) {
        if (Constants.REQUIRE.equalsIgnoreCase(verifyClient)) {
            sslConfig.setNeedClientAuth(true);
        } else if (Constants.OPTIONAL.equalsIgnoreCase(verifyClient)) {
    ...
    public SSLConfig getClientSSLConfig() {
        if (scheme == null || !scheme.equalsIgnoreCase(Constants.HTTPS_SCHEME)) {
            return null;
        }
    ...
    private SSLConfig getSSLConfigForListener() {
        ...
        String sslProtocol = sslConfig.getSSLProtocol() != null ? sslConfig.getSSLProtocol() : Constants.TLS_PROTOCOL;
        sslConfig.setSSLProtocol(sslProtocol);
        String tlsStoreType = sslConfig.getTLSStoreType() != null ? sslConfig.getTLSStoreType() : Constants.JKS;
        sslConfig.setTLSStoreType(tlsStoreType);
    ...
    private SSLConfig getSSLConfigForSender() {
        ...
        if (parameters != null) {
            for (Parameter parameter : parameters) {
                switch (parameter.getName()) {
                    case Constants.CLIENT_SUPPORT_CIPHERS:
                        sslConfig.setCipherSuites(parameter.getValue());
                        break;
                    case Constants.CLIENT_SUPPORT_SSL_PROTOCOLS:
                        sslConfig.setEnableProtocols(parameter.getValue());
                        break;
                    case Constants.CLIENT_ENABLE_SESSION_CREATION:
                        sslConfig.setEnableSessionCreation(Boolean.parseBoolean(parameter.getValue()));
                        break;
                    default:
                        //do nothing
