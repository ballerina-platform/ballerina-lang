...
public type PoolConfiguration record {
    int maxActiveConnections = config:getAsInt("b7a.http.pool.maxActiveConnections", defaultVal = -1);
    int maxIdleConnections = config:getAsInt("b7a.http.pool.maxIdleConnections", defaultVal = 100);
    int waitTimeinMillis = config:getAsInt("b7a.http.pool.waitTimeinMillis", defaultVal = 30000);
    int maxActiveStreamsPerConnection = config:getAsInt("b7a.http.pool.maxActiveStreamsPerConnection", defaultVal = 50);
};
