...
function getRedirectMethod(HttpOperation httpVerb, Response response) returns HttpOperation|() {
    ...
    if ((statusCode == STATUS_300 || statusCode == STATUS_305 || statusCode == STATUS_307
            || statusCode == STATUS_308) && (httpVerb == HTTP_GET || httpVerb == HTTP_HEAD)) {
        return httpVerb;
    } else if ((statusCode == STATUS_301 || statusCode == STATUS_302) &&
        (httpVerb == HTTP_GET || httpVerb == HTTP_HEAD)) {
        return HTTP_GET;
    } else if (statusCode == STATUS_303) {
        return HTTP_GET;
    } else {
        return ();
    }
