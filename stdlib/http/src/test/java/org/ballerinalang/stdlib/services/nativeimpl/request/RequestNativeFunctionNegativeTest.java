...
public class RequestNativeFunctionNegativeTest {
    ...
    public void testGetJsonPayloadWithoutPayload() {
        ...
        Assert.assertEquals(((BError) returnVals[0]).getDetails().stringValue(), "{message:\"No payload\", "
                + "cause:{ballerina/mime}ParsingEntityBodyFailed {message:\"Error occurred while extracting json data "
                + "from entity\", cause:{ballerina/mime}NoContentError {message:\"Empty content\"}}}");
    }
    ...
    public void testGetTextPayloadNegative() {
        ...
        Assert.assertTrue(returnVals[0].stringValue()
                        .contains("{ballerina/http}GenericClientError {message:\"No payload\", "
                                + "cause:{ballerina/mime}ParsingEntityBodyFailed {message:\"Error occurred while"
                                + " extracting text data from entity\", cause:{ballerina/mime}NoContentError "
    ...
    public void testGetXmlPayloadNegative() {
        ...
        Assert.assertEquals(((BError) returnVals[0]).getDetails().stringValue(), "{message:\"No payload\", "
                + "cause:{ballerina/mime}ParsingEntityBodyFailed {message:\"Error occurred while extracting xml data "
                + "from entity\", cause:{ballerina/mime}NoContentError {message:\"Empty content\"}}}");
    }
