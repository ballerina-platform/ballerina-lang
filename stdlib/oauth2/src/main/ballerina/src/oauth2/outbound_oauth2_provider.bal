...
# + oauth2Cache - Cached token configurations
    ...
    *auth:OutboundAuthProvider;
    ...
    public function generateToken() returns @tainted (string|auth:Error) {
        ...
        if (oauth2ProviderConfig is ()) {
            ...
            var authToken = generateAuthTokenForOAuth2(oauth2ProviderConfig, self.oauth2Cache);
            if (authToken is string) {
                return authToken;
            } else {
    ...
    public function inspect(map<anydata> data) returns @tainted (string|auth:Error?) {
        ...
        if (oauth2ProviderConfig is ()) {
            ...
            if (data[http:STATUS_CODE] == http:STATUS_UNAUTHORIZED) {
                var authToken = inspectAuthTokenForOAuth2(oauth2ProviderConfig, self.oauth2Cache);
                if (authToken is string) {
                    return authToken;
                } else {
...
# The `OAuth2Cache` stores the values received from the authorization/token server to use them
# for the latter requests without requesting tokens again.
#
# + accessToken - Access token for the  authorization endpoint
# + refreshToken - Refresh token for the refresh token server
# + expiryTime - Expiry time of the access token in milliseconds
public type OAuth2Cache record {
    string accessToken;
    string refreshToken;
    int expiryTime;
};
...
function generateAuthTokenForOAuth2(ClientCredentialsGrantConfig|PasswordGrantConfig|DirectTokenConfig authConfig,
                                    @tainted OAuth2Cache oauth2Cache) returns @tainted (string|Error) {
    if (authConfig is PasswordGrantConfig) {
        return getAuthTokenForOAuth2PasswordGrant(authConfig, oauth2Cache);
    } else if (authConfig is ClientCredentialsGrantConfig) {
        return getAuthTokenForOAuth2ClientCredentialsGrant(authConfig, oauth2Cache);
    } else {
        return getAuthTokenForOAuth2DirectTokenMode(authConfig, oauth2Cache);
    }
}
...
function getAuthTokenForOAuth2PasswordGrant(PasswordGrantConfig grantTypeConfig,
                                            @tainted OAuth2Cache oauth2Cache) returns @tainted (string|Error) {
    string cachedAccessToken = oauth2Cache.accessToken;
    if (cachedAccessToken == "") {
        string accessToken = check getAccessTokenFromAuthorizationRequest(grantTypeConfig, oauth2Cache);
        log:printDebug(function () returns string {
            return "OAuth2 password grant type; Access token received from authorization request. Cache is empty.";
        });
        return accessToken;
    } else {
        if (isCachedTokenValid(oauth2Cache)) {
            log:printDebug(function () returns string {
            ...
            lock {
                if (isCachedTokenValid(oauth2Cache)) {
                    cachedAccessToken = oauth2Cache.accessToken;
                    log:printDebug(function () returns string {
                        return "OAuth2 password grant type; Access token received from cache.";
                    });
                    return cachedAccessToken;
                } else {
                    string accessToken = check getAccessTokenFromRefreshRequest(grantTypeConfig, oauth2Cache);
                    log:printDebug(function () returns string {
...
function getAuthTokenForOAuth2ClientCredentialsGrant(ClientCredentialsGrantConfig grantTypeConfig,
                                                     @tainted OAuth2Cache oauth2Cache)
...
function isCachedTokenValid(OAuth2Cache oauth2Cache) returns boolean {
    int expiryTime = oauth2Cache.expiryTime;
    if (expiryTime == 0) {
        log:printDebug(function () returns string {
...
function getAccessTokenFromAuthorizationRequest(ClientCredentialsGrantConfig|PasswordGrantConfig config,
    ...
    return doRequest(tokenUrl, authorizationRequest, clientConfig, oauth2Cache, clockSkewInSeconds);
}
...
function getAccessTokenFromRefreshRequest(PasswordGrantConfig|DirectTokenConfig config,
    ...
    if (config is PasswordGrantConfig) {
        ...
        if (refreshConfig is RefreshConfig) {
            ...
            if (clientId is string && clientSecret is string) {
                ...
                requestConfig = {
                    payload: "grant_type=refresh_token&refresh_token=" + oauth2Cache.refreshToken,
                    clientId: clientId,
                    clientSecret: clientSecret,
    ...
    return doRequest(refreshUrl, refreshRequest, clientConfig, oauth2Cache, clockSkewInSeconds);
}
...
function doRequest(string url, http:Request request, http:ClientConfiguration clientConfig,
                   @tainted OAuth2Cache oauth2Cache, int clockSkewInSeconds) returns @tainted (string|Error) {
    ...
    if (response is http:Response) {
        ...
        return extractAccessTokenFromResponse(response, oauth2Cache, clockSkewInSeconds);
    } else {
...
function extractAccessTokenFromResponse(http:Response response, @tainted OAuth2Cache oauth2Cache, int clockSkewInSeconds)
                                        returns @tainted (string|Error) {
    if (response.statusCode == http:STATUS_OK) {
        ...
        if (payload is json) {
            ...
            updateTokenCache(payload, oauth2Cache, clockSkewInSeconds);
...
# + oauth2Cache - Cached token configurations
# + clockSkewInSeconds - Clock skew in seconds
function updateTokenCache(json responsePayload, OAuth2Cache oauth2Cache, int clockSkewInSeconds) {
    int issueTime = time:currentTime().time;
    string accessToken = responsePayload.access_token.toString();
    oauth2Cache.accessToken = accessToken;
    var expiresIn = responsePayload.expires_in;
    if (expiresIn is int) {
        oauth2Cache.expiryTime = issueTime + (expiresIn - clockSkewInSeconds) * 1000;
    }
    if (responsePayload.refresh_token is string) {
        string refreshToken = responsePayload.refresh_token.toString();
        oauth2Cache.refreshToken = refreshToken;
    }
    log:printDebug(function () returns string {
