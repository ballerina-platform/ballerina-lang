    ...
    while (!isDone) {
        ...
        if (readResult is string) {
            ...
            if (e is io:EofError) {
                ...
                io:GenericError readError = io:GenericError(io:GENERIC_ERROR, message = "Error while reading the content", cause = readResult);
                return readError;
            }
        }
    }
...
function writeCharacters(string content, int startOffset) returns int|io:Error? {
    ...
    // error e = error("Character channel not initialized properly");
    io:GenericError e = io:GenericError(io:GENERIC_ERROR, message = "Character channel not initialized properly");
    return e;
}
