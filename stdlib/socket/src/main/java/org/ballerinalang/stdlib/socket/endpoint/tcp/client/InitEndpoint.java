...
public class InitEndpoint extends BlockingNativeCallableUnit {
    ...
    public void execute(Context context) {
        try {
            ...
            Struct clientEndpointConfig = clientEndpoint.getStructField(SocketConstants.CLIENT_CONFIG);
            Value clientServiceType = clientEndpointConfig.getTypeField(SocketConstants.CLIENT_SERVICE_CONFIG);
            if (clientServiceType != null) {
                ...
                if (!"ballerina/socket:Client".equals(service.getEndpointName())) {
                    throw new BallerinaConnectorException(
                            "The callback service should be of type ");
                }
            }
            SocketChannel socketChannel = SocketChannel.open();
            socketChannel.configureBlocking(false);
            socketChannel.socket().setReuseAddress(true);
            clientEndpoint.addNativeData(SocketConstants.SOCKET_KEY, socketChannel);
            BMap<String, BValue> endpointConfig = (BMap<String, BValue>) context.getRefArgument(1);
            clientEndpoint.addNativeData(SocketConstants.CLIENT_CONFIG, endpointConfig);
