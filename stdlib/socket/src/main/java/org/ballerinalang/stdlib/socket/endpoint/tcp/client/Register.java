...
/**
 * Register client socket endpoint.
 *
 * @since 0.983.0
 */
@BallerinaFunction(
...
public class Register extends BlockingNativeCallableUnit {
    ...
    public void execute(Context context) {
        try {
            ...
            selectorManager.registerChannel(socketService, OP_CONNECT);
            context.setReturnValues();
        } catch (SelectorInitializeException e) {
            log.error(e.getMessage(), e);
            throw new BallerinaException("Unable to initialize the selector");
        } catch (ClosedChannelException e) {
            throw new BallerinaException("Socket not in an open state");
        }  catch (CancelledKeyException e) {
            throw new BallerinaException("Socket registration is failed");
        }
    }

    private SocketService getSocketService(Context context) {
        Map<String, Resource> resources = getResourceMap(context);
        SocketChannel serverSocket = getSocketChannel(context);
        return new SocketService(serverSocket, resources);
    }

    private SocketChannel getSocketChannel(Context context) {
        Struct clientEndpoint = BLangConnectorSPIUtil.getConnectorEndpointStruct(context);
        return (SocketChannel) clientEndpoint.getNativeData(SOCKET_KEY);
    }
