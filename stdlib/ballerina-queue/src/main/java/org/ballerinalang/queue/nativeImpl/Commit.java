    ...
    public void execute(Context ctx, CallableUnitCallback callback) {
        ...
        if (null == callback) {
            throw new BallerinaException("JMS Commit function can only be used with JMS Messages. "
                                                 + QueueConstants.JMS_SESSION_ACKNOWLEDGEMENT_MODE
                                                 + " property is not found in the message.", ctx);
        }

        BStruct messageStruct = ((BStruct) ctx.getRefArgument(0));
        if (messageStruct.getNativeData(QueueConstants.INBOUND_REQUEST) != null && !(Boolean) messageStruct
                .getNativeData(QueueConstants.INBOUND_REQUEST)) {
            throw new BallerinaException("JMS Commit function can only be used with Inbound JMS Messages.", ctx);
        }

        Object jmsSessionAcknowledgementMode = ctx.getProperties().get(QueueConstants.JMS_SESSION_ACKNOWLEDGEMENT_MODE);
        if (!(jmsSessionAcknowledgementMode instanceof Integer)) {
            throw new BallerinaException(
                    QueueConstants.JMS_SESSION_ACKNOWLEDGEMENT_MODE + " property should hold a " + "integer value. ");
        }
