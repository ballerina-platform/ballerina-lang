...
@BallerinaFunction(
        ...
        functionName = "LLVMCreateObjectFile",
        args = {
                @Argument(name = "memBuf", type = RECORD, structType = "LLVMMemoryBufferRef"),
        },
        returnType = {
                @ReturnType(type = RECORD, structType = "LLVMObjectFileRef", structPackage = "ballerina/llvm"),
        }
)
public class LLVMCreateObjectFile extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMMemoryBufferRef memBuf = FFIUtil.getRecodeArgumentNative(context, 0);
        LLVMObjectFileRef returnValue = LLVMCreateObjectFile(memBuf);
        BMap<String, BValue> rerunWrapperRecode = FFIUtil.newRecord(context, "LLVMObjectFileRef");
