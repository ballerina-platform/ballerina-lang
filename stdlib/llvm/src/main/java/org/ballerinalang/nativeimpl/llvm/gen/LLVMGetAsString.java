...
import static org.bytedeco.javacpp.LLVM.LLVMGetAsString;
...
@BallerinaFunction(
        ...
        functionName = "LLVMGetAsString",
        args = {
                @Argument(name = "c", type = RECORD, structType = "LLVMValueRef"),
                @Argument(name = "length", type = RECORD, structType = "SizeTPointer"),
        },
        returnType = {
                @ReturnType(type = RECORD, structType = "BytePointer", structPackage = "ballerina/llvm"),
        }
)
public class LLVMGetAsString extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMValueRef c = FFIUtil.getRecodeArgumentNative(context, 0);
        SizeTPointer length = FFIUtil.getRecodeArgumentNative(context, 1);
        BytePointer returnValue = LLVMGetAsString(c, length);
