...
import static org.bytedeco.javacpp.LLVM.LLVMMetadataAsValue;
...
@BallerinaFunction(
        ...
        functionName = "LLVMMetadataAsValue",
        args = {
                @Argument(name = "c", type = RECORD, structType = "LLVMContextRef"),
                @Argument(name = "md", type = RECORD, structType = "LLVMMetadataRef"),
        },
        returnType = {
                @ReturnType(type = RECORD, structType = "LLVMValueRef", structPackage = "ballerina/llvm"),
        }
)
public class LLVMMetadataAsValue extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMContextRef c = FFIUtil.getRecodeArgumentNative(context, 0);
        LLVM.LLVMMetadataRef md = FFIUtil.getRecodeArgumentNative(context, 1);
        LLVMValueRef returnValue = LLVMMetadataAsValue(c, md);
