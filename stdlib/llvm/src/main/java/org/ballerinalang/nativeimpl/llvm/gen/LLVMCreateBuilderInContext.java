...
@BallerinaFunction(
        ...
        functionName = "LLVMCreateBuilderInContext",
        args = {
                @Argument(name = "c", type = RECORD, structType = "LLVMContextRef"),
        },
        returnType = {
                @ReturnType(type = RECORD, structType = "LLVMBuilderRef", structPackage = "ballerina/llvm"),
        }
)
public class LLVMCreateBuilderInContext extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMContextRef c = FFIUtil.getRecodeArgumentNative(context, 0);
        LLVMBuilderRef returnValue = LLVMCreateBuilderInContext(c);
        BMap<String, BValue> rerunWrapperRecode = FFIUtil.newRecord(context, "LLVMBuilderRef");
