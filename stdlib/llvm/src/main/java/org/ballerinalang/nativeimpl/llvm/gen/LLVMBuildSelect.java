...
@BallerinaFunction(
        ...
        functionName = "LLVMBuildSelect",
        args = {
                @Argument(name = "arg0", type = RECORD, structType = "LLVMBuilderRef"),
                @Argument(name = "ifValue", type = RECORD, structType = "LLVMValueRef"),
                @Argument(name = "then", type = RECORD, structType = "LLVMValueRef"),
                @Argument(name = "elseValue", type = RECORD, structType = "LLVMValueRef"),
...
public class LLVMBuildSelect extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMBuilderRef arg0 = FFIUtil.getRecodeArgumentNative(context, 0);
        LLVM.LLVMValueRef ifValue = FFIUtil.getRecodeArgumentNative(context, 1);
        LLVM.LLVMValueRef then = FFIUtil.getRecodeArgumentNative(context, 2);
        LLVM.LLVMValueRef elseValue = FFIUtil.getRecodeArgumentNative(context, 3);
        String name = context.getStringArgument(0);
        LLVMValueRef returnValue = LLVMBuildSelect(arg0, ifValue, then, elseValue, name);
