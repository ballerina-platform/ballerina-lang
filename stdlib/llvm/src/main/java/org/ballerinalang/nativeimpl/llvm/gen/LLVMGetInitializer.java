...
@BallerinaFunction(
        ...
        functionName = "LLVMGetInitializer",
        args = {
                @Argument(name = "globalVar", type = RECORD, structType = "LLVMValueRef"),
        },
        returnType = {
                @ReturnType(type = RECORD, structType = "LLVMValueRef", structPackage = "ballerina/llvm"),
        }
)
public class LLVMGetInitializer extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMValueRef globalVar = FFIUtil.getRecodeArgumentNative(context, 0);
        LLVMValueRef returnValue = LLVMGetInitializer(globalVar);
