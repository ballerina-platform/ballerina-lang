...
@BallerinaFunction(
        ...
        functionName = "LLVMIntTypeInContext",
        args = {
                @Argument(name = "c", type = RECORD, structType = "LLVMContextRef"),
                @Argument(name = "numBits", type = INT),
        },
        returnType = {
                @ReturnType(type = RECORD, structType = "LLVMTypeRef", structPackage = "ballerina/llvm"),
        }
)
public class LLVMIntTypeInContext extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMContextRef c = FFIUtil.getRecodeArgumentNative(context, 0);
        int numBits = (int) context.getIntArgument(0);
        LLVMTypeRef returnValue = LLVMIntTypeInContext(c, numBits);
