...
import static org.bytedeco.javacpp.LLVM.LLVMBuildVAArg;
...
@BallerinaFunction(
        ...
        functionName = "LLVMBuildVAArg",
        args = {
                @Argument(name = "arg0", type = RECORD, structType = "LLVMBuilderRef"),
                @Argument(name = "list", type = RECORD, structType = "LLVMValueRef"),
...
public class LLVMBuildVAArg extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMBuilderRef arg0 = FFIUtil.getRecodeArgumentNative(context, 0);
        LLVM.LLVMValueRef list = FFIUtil.getRecodeArgumentNative(context, 1);
        LLVM.LLVMTypeRef ty = FFIUtil.getRecodeArgumentNative(context, 2);
        String name = context.getStringArgument(0);
        LLVMValueRef returnValue = LLVMBuildVAArg(arg0, list, ty, name);
