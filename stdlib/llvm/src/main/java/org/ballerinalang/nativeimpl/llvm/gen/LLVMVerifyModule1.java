...
@BallerinaFunction(
        ...
        functionName = "LLVMVerifyModule1",
        args = {
                @Argument(name = "m", type = RECORD, structType = "LLVMModuleRef"),
                @Argument(name = "action", type = INT),
...
public class LLVMVerifyModule1 extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMModuleRef m = FFIUtil.getRecodeArgumentNative(context, 0);
        int action = (int) context.getIntArgument(0);
        PointerPointer outMessage = FFIUtil.getRecodeArgumentNative(context, 1);
        int returnValue = LLVMVerifyModule(m, action, outMessage);
