...
public class LLVMBuildFence extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMBuilderRef b = FFIUtil.getRecodeArgumentNative(context, 0);
        int ordering = (int) context.getIntArgument(0);
        int singleThread = (int) context.getIntArgument(1);
        String name = context.getStringArgument(0);
        LLVMValueRef returnValue = LLVMBuildFence(b, ordering, singleThread, name);
