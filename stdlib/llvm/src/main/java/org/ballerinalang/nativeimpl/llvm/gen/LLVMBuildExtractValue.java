...
@BallerinaFunction(
        ...
        functionName = "LLVMBuildExtractValue",
        args = {
                @Argument(name = "arg0", type = RECORD, structType = "LLVMBuilderRef"),
                @Argument(name = "aggVal", type = RECORD, structType = "LLVMValueRef"),
...
public class LLVMBuildExtractValue extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMBuilderRef arg0 = FFIUtil.getRecodeArgumentNative(context, 0);
        LLVM.LLVMValueRef aggVal = FFIUtil.getRecodeArgumentNative(context, 1);
        int index = (int) context.getIntArgument(0);
        String name = context.getStringArgument(0);
        LLVMValueRef returnValue = LLVMBuildExtractValue(arg0, aggVal, index, name);
