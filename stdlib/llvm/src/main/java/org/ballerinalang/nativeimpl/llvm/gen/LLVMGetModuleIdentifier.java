...
@BallerinaFunction(
        ...
        functionName = "LLVMGetModuleIdentifier",
        args = {
                @Argument(name = "m", type = RECORD, structType = "LLVMModuleRef"),
                @Argument(name = "len", type = RECORD, structType = "SizeTPointer"),
        },
        returnType = {
                @ReturnType(type = RECORD, structType = "BytePointer", structPackage = "ballerina/llvm"),
        }
)
public class LLVMGetModuleIdentifier extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMModuleRef m = FFIUtil.getRecodeArgumentNative(context, 0);
        SizeTPointer len = FFIUtil.getRecodeArgumentNative(context, 1);
        BytePointer returnValue = LLVMGetModuleIdentifier(m, len);
