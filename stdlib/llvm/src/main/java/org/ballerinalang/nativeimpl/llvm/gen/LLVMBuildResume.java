...
import static org.bytedeco.javacpp.LLVM.LLVMBuildResume;
...
@BallerinaFunction(
        ...
        functionName = "LLVMBuildResume",
        args = {
                @Argument(name = "b", type = RECORD, structType = "LLVMBuilderRef"),
                @Argument(name = "exn", type = RECORD, structType = "LLVMValueRef"),
        },
        returnType = {
                @ReturnType(type = RECORD, structType = "LLVMValueRef", structPackage = "ballerina/llvm"),
        }
)
public class LLVMBuildResume extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMBuilderRef b = FFIUtil.getRecodeArgumentNative(context, 0);
        LLVM.LLVMValueRef exn = FFIUtil.getRecodeArgumentNative(context, 1);
        LLVMValueRef returnValue = LLVMBuildResume(b, exn);
