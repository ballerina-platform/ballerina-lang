...
import static org.bytedeco.javacpp.LLVM.LLVMBuildLandingPad;
...
@BallerinaFunction(
        ...
        functionName = "LLVMBuildLandingPad",
        args = {
                @Argument(name = "b", type = RECORD, structType = "LLVMBuilderRef"),
                @Argument(name = "ty", type = RECORD, structType = "LLVMTypeRef"),
                @Argument(name = "persFn", type = RECORD, structType = "LLVMValueRef"),
                @Argument(name = "numClauses", type = INT),
...
public class LLVMBuildLandingPad extends BlockingNativeCallableUnit {
    ...
    public void execute(Context context) {
        ...
        LLVM.LLVMValueRef persFn = FFIUtil.getRecodeArgumentNative(context, 2);
        int numClauses = (int) context.getIntArgument(0);
        String name = context.getStringArgument(0);
        LLVMValueRef returnValue = LLVMBuildLandingPad(b, ty, persFn, numClauses, name);
