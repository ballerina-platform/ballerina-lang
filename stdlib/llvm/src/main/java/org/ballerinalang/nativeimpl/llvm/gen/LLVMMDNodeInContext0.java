...
@BallerinaFunction(
        ...
        functionName = "LLVMMDNodeInContext0",
        args = {
                @Argument(name = "c", type = RECORD, structType = "LLVMContextRef"),
                @Argument(name = "vals", type = RECORD, structType = "PointerPointer"),
...
public class LLVMMDNodeInContext0 extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMContextRef c = FFIUtil.getRecodeArgumentNative(context, 0);
        PointerPointer vals = FFIUtil.getRecodeArgumentNative(context, 1);
        int count = (int) context.getIntArgument(0);
        LLVMValueRef returnValue = LLVMMDNodeInContext(c, vals, count);
