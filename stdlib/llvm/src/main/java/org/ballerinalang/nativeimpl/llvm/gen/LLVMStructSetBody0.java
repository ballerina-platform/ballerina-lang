...
@BallerinaFunction(
        ...
        functionName = "LLVMStructSetBody0",
        args = {
                @Argument(name = "structTy", type = RECORD, structType = "LLVMTypeRef"),
                @Argument(name = "elementTypes", type = RECORD, structType = "LLVMTypeRef"),
                @Argument(name = "elementCount", type = INT),
                @Argument(name = "packed", type = INT),
        })
public class LLVMStructSetBody0 extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        LLVM.LLVMTypeRef structTy = FFIUtil.getRecodeArgumentNative(context, 0);
        LLVM.LLVMTypeRef elementTypes = FFIUtil.getRecodeArgumentNative(context, 1);
