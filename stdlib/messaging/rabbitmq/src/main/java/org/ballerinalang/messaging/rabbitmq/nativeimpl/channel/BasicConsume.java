...
@BallerinaFunction(
        ...
        functionName = "basicConsume",
        receiver = @Receiver(type = TypeKind.OBJECT,
                structType = CHANNEL_STRUCT, structPackage = STRUCT_PACKAGE_RABBITMQ),
        isPublic = true
)
public class BasicConsume implements NativeCallableUnit {
    @Override
    public void execute(Context context, CallableUnitCallback callback) {
        BMap<String, BValue> channelBObject = (BMap<String, BValue>) context.getRefArgument(0);
        Channel channel = RabbitMQUtils.getNativeObject(channelBObject, CHANNEL_NATIVE_OBJECT, Channel.class, context);
    }
