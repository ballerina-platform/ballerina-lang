...
@BallerinaFunction(
        ...
        receiver = @Receiver(type = TypeKind.OBJECT,
                structType = CHANNEL_STRUCT,
                structPackage = STRUCT_PACKAGE_RABBITMQ)
)
public class CreateChannel implements NativeCallableUnit {
    ...
    public void execute(Context context, CallableUnitCallback callback) {
        ...
        Connection connection = RabbitMQUtils.getNativeObject(connectionBObject, CONNECTION_NATIVE_OBJECT,
                Connection.class, context);
        Channel channel = ChannelUtils.createChannel(connection);
        channelBObject.addNativeData(CHANNEL_NATIVE_OBJECT, channel);
    }
