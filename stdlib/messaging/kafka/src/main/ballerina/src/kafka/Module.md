...
>**Note:** The Kafka module is not designed to use the Ballerina non-blocking thread model. The default thread pool size used in Ballerina is number of processers available * 2. You can configure the thread pool size by using the `BALLERINA_MAX_POOL_SIZE` environment variable.
