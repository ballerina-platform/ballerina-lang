...
public class GetAuthToken extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        AuthenticationContext authContext = AuthenticationContextUtils.getAuthenticationContext(context);
        String authToken = authContext.getAuthToken();
        if (authToken == null) {
            authToken = BTypes.typeString.getZeroValue().stringValue();
        }
        context.setReturnValues(new BString(authToken));
    }
}
