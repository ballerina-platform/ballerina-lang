...
public class GetUserGroups extends BlockingNativeCallableUnit {

    @Override
    public void execute(Context context) {
        AuthenticationContext authContext = AuthenticationContextUtils.getAuthenticationContext(context);
        String[] userGroups = authContext.getGroups();
        if (userGroups == null) {
            userGroups = new String[0];
        }
        context.setReturnValues(new BStringArray(userGroups));
    }
}
