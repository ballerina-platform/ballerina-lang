    ...
    resource hub (http:ServerConnector conn, http:Request request) {
        ...
        if (entityError == null) {
            ...
            if (mode != null) {
                ...
                if (mode == http:MODE_PUBLISH) {
                    topic, _ = (string) params[http:HUB_TOPIC];
                    json payload;
            ...
            if (validSubscriptionRequest) {
                var callback, _ = (string) params[http:HUB_CALLBACK];
...
function validateSubscriptionChangeRequest(map params) (boolean, string) {
    ...
    if (topic != null && callback != null) {
        ...
        if (mode == http:MODE_SUBSCRIBE || mode == http:MODE_UNSUBSCRIBE) {
...
function verifyIntent(string callback, map params) {
    ...
    var mode, _ = (string) params[http:HUB_MODE];
    var topic, _ = (string) params[http:HUB_TOPIC];
    var secret, _ = (string) params[http:HUB_SECRET];
    var leaseSeconds, _ = (int) params[http:HUB_LEASE_SECONDS];
    ...
    string queryParams = http:HUB_MODE + "=" + mode
                         + "&" + http:HUB_TOPIC + "=" + topic
...
public function distributeContent(string callback, SubscriptionDetails subscriptionDetails, json payload) {
    ...
    if (currentTime - leaseSeconds > createdAt) {
        ...
        if (subscriptionDetails.secret != "null") {
            ...
            request.setHeader(http:X_HUB_SIGNATURE, xHubSignature);
        }
