...
//        ClientCalls.ThreadlessExecutor executor = new ClientCalls.ThreadlessExecutor();
    ...
    // TODO(louiscryan): Not clear if we want to use this idiom for 'simple' stubs.
    public static <ReqT, RespT> Iterator<RespT> blockingServerStreamingCall(
            HttpClientConnector connector, MethodDescriptor<ReqT, RespT> method, CallOptions callOptions, ReqT param,
            CallableUnitCallback callback) {

        ThreadlessExecutor executor = new ThreadlessExecutor();
        OutboundMessage outboundMessage = new OutboundMessage(MessageUtils.createHttpCarbonMessage(true));
        ClientCall<ReqT, RespT> call = new ClientCallImpl<>(connector, outboundMessage, method, callOptions, callback);
    ...
    private static <V> V getUnchecked(Future<V> future) {
        ...
        try {
            ...
            throw Status.Code.CANCELLED.toStatus()
    ...
    private static <ReqT, RespT> void startCall(ClientCall<ReqT, RespT> call, ClientCall.Listener<RespT>
            responseListener, boolean streamingResponse) {
    ...
    private static final class StreamObserverToCallListenerAdapter<ReqT, RespT>
        ...
        public void onMessage(RespT message) {
            ...
            if (streamingResponse && adapter.autoFlowControlEnabled) {
                // InboundMessage delivery of the next inbound message.
                adapter.request(1);
            }
        }
            ...
            if (status.isOk()) {
                if (value == null) {
                    ...
                    responseFuture.setException(
                            Status.Code.INTERNAL.toStatus().withDescription("No value received for unary call")
                                    .asRuntimeException());
                }
    ...
    /**
     * Convert events on a {@link ClientCall.Listener} into a blocking
     * {@link Iterator}.
     * <p>
    ...
    private static final class BlockingResponseStream<T> implements Iterator<T> {
        ...
        public boolean hasNext() {
            ...
            if (last == null) {
                try {
                    ...
                    Thread.currentThread().interrupt();
                    throw Status.Code.CANCELLED.toStatus().withDescription("interrupted").withCause(ie)
                            .asRuntimeException();
                }
            }
        ...
        private final class QueuingListener extends ClientCall.Listener<T> {
            ...
            public void onClose(Status status, HttpHeaders trailers) {
                ...
                Preconditions.checkState(!done, "ClientCall already closed");
                if (status.isOk()) {
                    buffer.add(ClientCalls.BlockingResponseStream.this);
                } else {
