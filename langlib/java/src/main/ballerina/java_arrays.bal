...
# ```ballerina
# handle stringClass = check getClass("java.lang.String");
...
public function newInstance(handle classType, int ...dimensions) returns handle = @Method {
...
public function get(handle array, int index) returns handle = @Method {
    'class: "java.lang.reflect.Array"
} external;
...
# java.arrays:set(strArray, 0, fromString("Ballerina"));
# ```
#
# + array - The `handle`, which refers to the Java array
# + index - The index of the element to be replaced
# + element - The element to be stored at the specified index
public function set(handle array, int index, handle element) = @Method {
    'class: "java.lang.reflect.Array"
} external;
...
public function getLength(handle array) returns int = @Method {
    'class: "java.lang.reflect.Array"
} external;
    ...
    if (!isNull(array)) {
        if (jType == "string") {
            ...
            foreach int i in 0 ... count-1 {
                string? element = toString(<handle>get(array, i));
...
public function toHandle(any[] array, string jType) returns handle|error {
    handle returnHandle = newInstance(check getClass(jType), array.length());
    int count=0;
    while (count < array.length()) {
        if (array is byte[] && jType == "char") {
            ...
            set(returnHandle, count, fromString(array[count]));
            count+=1;
