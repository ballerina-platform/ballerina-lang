...
function testSubString() {
    string subs = str.substring(6, 9);
    test:assertEquals(subs, "Bal");
    subs = str.substring(6);
    test:assertEquals(subs, "Ballerina!");
    subs = strings:substring(str,6);
    test:assertEquals(subs, "Ballerina!");
}
...
function testIterator() {
    ...
    test:assertEquals(chars, ["F", "o", "o", " ", "B", "a", "r"]);
}
...
function testToCodePointIntsDataProvider(){
    [string, int, int[]][] data = [["", 0, []], ["a", 1, [97]], ["a👻cd", 4, [97, 128123, 99, 100]]];
    foreach [string, int, int[]] element in data {
        int[] b = testToCodePointInts(element[0]);
        test:assertEquals(b.length(), element[1]);
        test:assertEquals(b, element[2]);
    }
}
...
function testFromCodePointsIntNegativeDataProvider() {
    string|error returnValue = testFromCodePointInts([0x10FFFF, 0x10FFFF + 1]);
    test:assertError(returnValue);
    test:assertEquals(returnValue.toString(), "error Invalid codepoint: 1114112");
}

function testFromCodePointIntsDataProvider() {
    [int[], string][] data = [[[], ""], [[97], "a"], [[97, 128123, 99, 100], "a👻cd"]];
    foreach [int[], string] element in data {
        test:assertEquals(testFromCodePointInts(element[0]).toString(), element[1]);
    }
}
...
function testSubstringDataProvider(){
    string suffix = "error {ballerina/lang.string}StringOperationError message=";
    [string, int, int, string][] data = [["abcdef", -2, -1, "string index out of range. Length:'6' requested: '-2' to '-1'"],
     ...
     foreach  [string, int, int, string] element in data {
         string|error returnValue = testSubstring(element[0], element[1], element[2]);
         test:assertError(returnValue);
         test:assertEquals(returnValue.toString(), suffix + element[3]);
     }
}
