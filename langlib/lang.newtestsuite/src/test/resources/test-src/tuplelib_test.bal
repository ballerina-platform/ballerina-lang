...
function testEnumerate() {
    ...
    [int, (int|string|float)][] t = person.enumerate();
    test:assertEquals(t.length(), 4);
    test:assertEquals(t[0], [0, 1]);
    test:assertEquals(t[1], [1, "John Doe"]);
    test:assertEquals(t[2], [2, 25]);
    test:assertEquals(t[3], [3, 5.9]);
}
...
function testMap() {
    ...
    test:assertEquals(newArr[0], 10);
    test:assertEquals(newArr[1], "mapped John Doe");
    test:assertEquals(newArr[2], 250);
    test:assertEquals(newArr[3], 29.5);
}
...
function testSlice() {
    ...
    (int|string|float|boolean|map<string>)[] t = tup.slice(1, 4);
    test:assertEquals(t.length(), 3);
    test:assertEquals(t[0], "Foo");
    test:assertEquals(t[1], 12.34);
    test:assertTrue(<boolean> t[2]);
}
