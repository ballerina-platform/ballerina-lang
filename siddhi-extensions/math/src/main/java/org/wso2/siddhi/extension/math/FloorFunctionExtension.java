...
public class FloorFunctionExtension extends FunctionExecutor {
    private static final Logger log = Logger.getLogger(FloorFunctionExtension.class);
    ...
    protected Object execute(Object data) {
        ...
        if (data != null) {
            ...
            returnValue = Math.floor(inputValue);
            if (isDebugMode) {
                log.debug("Input Value = " + inputValue + ", Output Value =" + returnValue);
            }
        } else {
            throw new OperationNotSupportedException("Input to the math:floor() function cannot be null");
        }
        return returnValue;
    }
