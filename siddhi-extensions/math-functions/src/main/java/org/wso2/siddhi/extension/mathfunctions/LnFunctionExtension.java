...
public class LnFunctionExtension extends FunctionExecutor {
    ...
    private static final Logger log = Logger.getLogger(LnFunctionExtension.class);
    ...
    protected Object execute(Object data) {
        ...
        if (data != null) {
            ...
            returnValue = Math.log(inputValue);
            if (isDebugMode) {
                log.debug("Input Value = " + inputValue + ", Output Value =" + returnValue);
            }
        } else {
            throw new OperationNotSupportedException("Input to the math:ln() function cannot be null");
        }
        return returnValue;
    }
