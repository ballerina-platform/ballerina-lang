...
public class RepeatFunctionExtension extends FunctionExecutor {
    ...
    protected void init(ExpressionExecutor[] attributeExpressionExecutors, ExecutionPlanContext executionPlanContext) {
        ...
        if (attributeExpressionExecutors[1].getClass().toString().equals("class org.wso2.siddhi.core.executor.ConstantExpressionExecutor")) {
            times = (Integer) ((ConstantExpressionExecutor) attributeExpressionExecutors[1]).getValue();
        } else {
            isVaryingIndex = true;
        }
    }
    ...
    protected Object execute(Object[] data) {
        if (data[0] == null || data[1] == null) {
            throw new OperationNotSupportedException("Input to str:repeat() function cannot be null");
        }
        String source = (String) data[0];
        if (isVaryingIndex) {
            times = (Integer) data[1];
        }
        String output = "";
    ...
    public Object[] currentState() {
        return new Object[]{times, isVaryingIndex};
    }

    @Override
    public void restoreState(Object[] state) {
        times = (Integer) state[0];
