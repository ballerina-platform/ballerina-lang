...
/**
 * charat(string , times)
 * Returns the char value at the specified times.
...
public class CharAtFunctionExtension extends FunctionExecutor {
    ...
    protected void init(ExpressionExecutor[] attributeExpressionExecutors, ExecutionPlanContext executionPlanContext) {
        if (attributeExpressionExecutors.length != 2) {
            throw new ExecutionPlanCreationException("Invalid no of Arguments Passed. Required 2. Found " + attributeExpressionExecutors.length);
        }
        ...
        if (attributeExpressionExecutors[1].getClass().toString().equals("class org.wso2.siddhi.core.executor.ConstantExpressionExecutor")) {
            index = (Integer) ((ConstantExpressionExecutor) attributeExpressionExecutors[1]).getValue();
        } else {
            isVaryingIndex = true;
        }
    }
    ...
    protected Object execute(Object[] data) {
        if (data[0] == null || data[1] == null) {
            throw new OperationNotSupportedException("Input to str:charat() function cannot be null");
        }
        ...
        try {
            ...
            throw new OperationNotSupportedException("Index argument is negative or not less than the length of the given string", e);
        }
    }
    ...
    public Object[] currentState() {
        return new Object[]{index, isVaryingIndex};
    }

    @Override
    public void restoreState(Object[] state) {
        index = (Integer) state[0];
        isVaryingIndex = (Boolean) state[1];
    }
}
