...
/**
 * replace_first(string, target, replacement)
...
public class ReplaceFirstFunctionExtension extends FunctionExecutor {
    ...
    protected void init(ExpressionExecutor[] attributeExpressionExecutors, ExecutionPlanContext executionPlanContext) {
        if (attributeExpressionExecutors.length != 3) {
            throw new ExecutionPlanCreationException(
                    "str:replace_first function can have only three parameters");
        }
        if (attributeExpressionExecutors[0].getReturnType() != Attribute.Type.STRING
            ...
            throw new ExecutionPlanCreationException(
                    "str:replace_first function should take the form string:replace_all(<string attribute>, <string target>, <string replacement>)");
        }
    ...
    protected Object execute(Object[] data) {
        if (data[0] == null || data[1] == null || data[2] == null) {
            throw new OperationNotSupportedException("Input to str:replace_first() function cannot be null");
        }
        ...
        return source.replaceFirst(target, replacement);
    }
