...
public class RegexpFunctionExtension extends FunctionExecutor {
    ...
    protected void init(ExpressionExecutor[] attributeExpressionExecutors, ExecutionPlanContext executionPlanContext) {
        if (attributeExpressionExecutors.length != 2) {
            throw new ExecutionPlanCreationException(
                    "regexp function can have only two parameters");
        }
        if (attributeExpressionExecutors[0].getReturnType() != Attribute.Type.STRING
                || attributeExpressionExecutors[1].getReturnType() != Attribute.Type.STRING) {
            throw new ExecutionPlanCreationException(
                    "regexp function should take the form str:regexp(<string attribute> , <string regex>)");
        }
    }
    ...
    protected Object execute(Object[] data) {
        if (data[0] == null || data[1] == null) {
            throw new OperationNotSupportedException("Input to str:regex() function cannot be null");
        }
