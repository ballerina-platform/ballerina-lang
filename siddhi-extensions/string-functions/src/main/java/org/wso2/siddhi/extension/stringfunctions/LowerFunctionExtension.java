...
public class LowerFunctionExtension extends FunctionExecutor {
    ...
    protected void init(ExpressionExecutor[] attributeExpressionExecutors, ExecutionPlanContext executionPlanContext) {
        if (attributeExpressionExecutors.length != 1) {
            throw new ExecutionPlanCreationException("Lower function can have only one parameter");
        }
        if (attributeExpressionExecutors[0].getReturnType() != Attribute.Type.STRING) {
            throw new ExecutionPlanCreationException(
                    "Lower function can have only a string parameter");
        }
    }
    ...
    protected Object execute(Object data) {
        if (data == null) {
            throw new OperationNotSupportedException("Input to str:lower() function cannot be null");
        }
