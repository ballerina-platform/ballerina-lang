...
public class LengthFunctionExtension extends FunctionExecutor {
    ...
    protected void init(ExpressionExecutor[] attributeExpressionExecutors, ExecutionPlanContext executionPlanContext) {
        if (attributeExpressionExecutors.length != 1) {
            throw new ExecutionPlanCreationException(
                    "Length function can have only one parameter");
        } else if (attributeExpressionExecutors[0].getReturnType() != Attribute.Type.STRING) {
            throw new ExecutionPlanCreationException(
                    "Length function can have only a string parameter");
        }
    }
    ...
    protected Object execute(Object data) {
        if (data == null) {
            throw new OperationNotSupportedException("Input to str:length() function cannot be null");
        }
