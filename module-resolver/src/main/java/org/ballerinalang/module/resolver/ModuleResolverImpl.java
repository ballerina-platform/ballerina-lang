...
import org.ballerinalang.module.resolver.model.SingleBalEntity;
import org.ballerinalang.repository.PackageEntity;
import org.ballerinalang.toml.model.Dependency;
import org.wso2.ballerinalang.compiler.ModuleResolver;
...
import static org.ballerinalang.module.resolver.Util.isAbsoluteVersion;
...
public class ModuleResolverImpl implements ModuleResolver {
    ...
    private boolean isLockEnabled; // todo: Should add this after lock implementation of project api
    ...
    public ModuleResolverImpl(Project project, RepoHierarchyImpl repoHierarchy, boolean isLockEnabled, boolean testsEnabled,
    ...
    public PackageID resolveVersion(PackageID packageID, PackageID enclPackageID) {
        ...
        // if module is in the current pkg
        if (this.project.currentPackage().containsModule(packageID)) {
            packageID.version = new Name(this.project.currentPackage().ballerinaToml().getPackage().getVersion());
            resolvedModules.put(packageID, repoHierarchy.getProjectModules());
            return packageID;
        }

        // if module exists in pkg build repo, This applies in the build
        ...
        if (enclPackageID != null && this.project.currentPackage().ballerinaToml() != null) { // this.pkg.isModuleExists(enclPackageID)
            // If path balo dependency
            String pathBaloPath = resolvePathBaloFromManifest(packageID, this.project.currentPackage().ballerinaToml());
        ...
        // if lock file exists
//        if (enclPackageID != null && this.pkg.hasLockFile() && this.isLockEnabled) {
...
//        }
        ...
        // Set version from the Ballerina.toml of the current pkg
        if (enclPackageID != null && this.project.currentPackage().ballerinaToml() != null
                && this.project.currentPackage().containsModule(enclPackageID)) {
            // If exact version return
            versionFromManifest = resolveVersionFromManifest(packageID, this.project.currentPackage().ballerinaToml());
        ...
        // if enclosing module is not a pkg module, module is transitive
        // if it is a transitive we need to check the toml file of the dependent module
        if (enclPackageID != null && !this.project.currentPackage().containsModule(enclPackageID)) {
            ...
            if (parentModule != null) {
                BallerinaToml parentBallerinaToml = io.ballerina.projects.utils.RepoUtils
                        .getBallerinaTomlFromBalo(getBaloPath(parentModule.getSourcePath()));
                versionFromManifest = resolveVersionFromManifest(packageID, parentBallerinaToml);
                PackageID resolvedModuleId = getPackageIDFromRepos(packageID, versionFromManifest);
                if (resolvedModuleId != null) {
                    return resolvedModuleId;
                }
...
//        if (this.pkg.getLockFile().getImports().containsKey(enclModuleId.toString())) {
//            List<LockFileImport> foundBaseImport = this.pkg.getLockFile().getImports().get(enclModuleId.toString());
//
...
//    }
    ...
    private String resolvePathBaloFromManifest(PackageID moduleId, BallerinaToml ballerinaToml) {
        for (Dependency dependency : ballerinaToml.getDependencies()) {
    ...
    private String resolveVersionFromManifest(PackageID moduleId, BallerinaToml ballerinaToml) {
        for (Dependency dependency : ballerinaToml.getDependencies()) {
            if (dependency.getModuleName().equals(moduleId.getName().getValue())
                    && dependency.getOrgName().equals(moduleId.getOrgName().getValue())
