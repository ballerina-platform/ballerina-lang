...
function main (string[] args) {
    ...
    int ret = testDB.update("create table Students(id int auto_increment,
    ...
    params = [para1, para2];
    ret = testDB.update("Insert into Students (age,name) values (?,?)",
                       params);
    ...
    string[] keyColumns = [];
    string[] ids;
    ret, ids = testDB.updateWithGeneratedKeys("Insert into Students
    ...
    params = [para1];
    datatable dt = testDB.select("SELECT * from Students where age = ?",
                                 params);
    ...
    int[] count = testDB.batchUpdate("Insert into Students (age,name)
                                     values (?,?)", bPara);
