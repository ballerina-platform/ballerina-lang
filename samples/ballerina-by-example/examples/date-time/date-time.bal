...
function main (string[] args) {
    ...
    system:println("Current system time in milliseconds " +
                   "since epoch:" + currentTimeMills);
    ...
    system:println("Created Time:" +
                                time:toString(timeCreated));
    //Returns the time for the given string representation
    //based on the given format string
    time:Time timeParsed =
         time:parse("2017-06-26T09:46:22.444-0500",
                    "yyyy-MM-dd'T'HH:mm:ss.SSSZ");
    system:println("Parsed Time:" +
                                 time:toString(timeParsed));
    //String representation of time can be retrieved via
    //toString or format functions.Get the ISO 8601
    //formatted string of a given time
    string standardTimeString = time:toString(currentTime);
    system:println("Current system time in iso format:"
                   + standardTimeString);
    //Get the formatted string of a given time
    string customeTimeString = time:format(currentTime,
                                           "yyyy-MM-dd-E");
    system:println("Current system time in custom format:"
                   + customeTimeString);
    ...
    system:println("Time:" + hour + ":" + minute + ":"
                              + second + ":" + milliSecond);
    //Add a given duration to the time. Here we are adding
    //one year, one month and one second to the current time
    time:Time timeStructAdd = time:addDuration(currentTime,
                                       1, 1, 0, 0, 0, 1, 0);
    system:println("After add duration:" +
                            time:toString(timeStructAdd));
    //Subtract a given duration from the time. Here we are
    //subtracting one year, one month and one second from
    //the current time.
    time:Time timeStructSub = time:subtractDuration
                         (currentTime, 1, 1, 0, 0, 0, 1, 0);
    system:println("After subtract duration:" +
                   time:toString(timeStructSub));
}
