...
Connectors may need to have usage-specific parameters and hence, must be instantiated prior to their usage. For example, an HTTP connector will need the URL to connect to as a minimum.
...
A connector comes with a set of remote functions that represent the interactions that one can have with the remote service. Remote functions can be uni-directional or bi-directional and represent blocking from the calling worker's perspective. That is, the calling thread is blocked until the remote function has completed its remote interaction.
...
2. Drag and drop this onto the main program in the canvas. This automatically applies this function on message **m** since that is the default message.
    ![alt text](../images/GetJsonPayload.gif)
3. Modify the name of the JSON function you added to something meaningful.
    ![alt text](../images/GetJsonPayloadMessage.png)
4. To get the data under the list, you need to add a variable from the tool palette to the main function in the canvas.
    ![alt text](../images/JsonVariable.gif)
5. Modify the variable so that you can get the data under the list. In this example, **.list** is a JSON path. 
...
Use a tool that can interact with HTTP APIs. This instance uses Postman, which is a Google Chrome app. Paste the URL above in the **Enter request URL** text box and change the request type to **GET**. In the JSON response, you can see the **list** part of the response contains the temperature (**temp**) in Kelvin.
