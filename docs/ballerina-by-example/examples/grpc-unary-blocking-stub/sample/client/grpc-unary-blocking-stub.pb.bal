package sample.client;
import ballerina.net.grpc;

public connector helloWorldBlockingStub (string host, int port) {
    endpoint<grpc:GRPCConnector> ep {
        create grpc:GRPCConnector(host, port, "blocking", descriptorKey, descriptorMap);
    }

    action hello (string req) (string, error) {
        var res, err1 = ep.blockingExecute("server.org.ballerinalang.net.grpc.helloworld.helloWorld/hello", req);
        if (err1 != null) {
            error e = {message:err1.message};
            return null, e;
        }
        var response, err2 = (string)res;
        if (err2 != null) {
            error e = {message:err2.message};
            return null, e;
        }
        return response, null;
    }
}

public connector helloWorldNonBlockingStub (string host, int port) {
    endpoint<grpc:GRPCConnector> ep {
        create grpc:GRPCConnector(host, port, "non-blocking", descriptorKey, descriptorMap);
    }

    action hello (string req, string serviceName) (error) {
        var err1 = ep.nonBlockingExecute("server.org.ballerinalang.net.grpc.helloworld.helloWorld/hello", req, serviceName);
        if (err1 != null) {
            error e = {message:err1.message};
            return e;
        }
        return null;
    }
}


const string descriptorKey = "server.org.ballerinalang.net.grpc.helloworld.helloWorld.proto";
map descriptorMap = {
                        "server.org.ballerinalang.net.grpc.helloworld.helloWorld.proto":"",
                        "google.protobuf.google/protobuf/wrappers.proto":""};