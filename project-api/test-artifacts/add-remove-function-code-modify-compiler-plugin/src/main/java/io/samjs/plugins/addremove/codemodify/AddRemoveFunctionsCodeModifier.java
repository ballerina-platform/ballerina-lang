 ...
 * @since 2201.9.0
 */
public class AddRemoveFunctionsCodeModifier extends CodeModifier {

    @Override
    public void init(CodeModifierContext modifierContext) {
        modifierContext.addSourceModifierTask(context -> {
            for (ModuleId moduleId : context.currentPackage().moduleIds()) {
                io.ballerina.projects.Module module = context.currentPackage().module(moduleId);
                for (DocumentId documentId : module.documentIds()) {
                    Document document = module.document(documentId);
                    ModulePartNode rootNode = document.syntaxTree().rootNode();

                    // Remove empty functions
                    List<ModuleMemberDeclarationNode> emptyFunctionNodes = rootNode.members().stream()
                            .filter(member -> member.kind() == SyntaxKind.FUNCTION_DEFINITION)
                            .filter(member -> ((FunctionDefinitionNode) member).functionBody().children().size() < 3)
                            .toList();
                    NodeList<ModuleMemberDeclarationNode> updatedMembers =
                            rootNode.members().removeAll(emptyFunctionNodes);

                    // Add new function
                    updatedMembers = updatedMembers.add(createFunctionDefNode(document));
                    ModulePartNode newModulePart = rootNode.modify(
                            rootNode.imports(), updatedMembers, rootNode.eofToken());
