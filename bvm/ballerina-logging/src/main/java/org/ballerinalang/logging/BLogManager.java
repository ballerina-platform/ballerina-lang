...
public class BLogManager extends LogManager {
    ...
    public void setHttpTraceLogHandler() throws IOException, TraceLogConfigurationException {
        ...
        ConfigRegistry configRegistry = ConfigRegistry.getInstance();
        String logTo = configRegistry.getInstanceConfigValue(HTTP_TRACE_LOG, LOG_TO);
    ...
    private Handler populateTraceHandlerConfiguration(String logToString, ConfigRegistry configRegistry)
        ...
        if (logToString.equalsIgnoreCase(Constants.LOG_TO_CONSOLE)) {
            handler = new ConsoleHandler();
            handler.setFormatter(new HTTPTraceLogFormatter());
        } else if (logToString.equalsIgnoreCase(Constants.LOG_TO_SOCKET)) {
            String host = configRegistry.getInstanceConfigValue(HTTP_TRACE_LOG, Constants.LOG_PUBLISH_HOST);
            int port = Integer
                    .parseInt(configRegistry.getInstanceConfigValue(HTTP_TRACE_LOG, Constants.LOG_PUBLISH_PORT));
