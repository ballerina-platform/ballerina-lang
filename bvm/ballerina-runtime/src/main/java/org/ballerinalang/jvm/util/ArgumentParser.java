...
public class ArgumentParser {
    ...
    public static Object[] extractEntryFuncArgs(RuntimeUtils.ParamInfo[] funcInfo, String[] args,
        ...
        int defaultableIndex = 1;
        int defaultableCount = 0;
        for (RuntimeUtils.ParamInfo info : funcInfo) {
            if (info.hasDefaultable) {
                info.index = defaultableIndex++;
                namedArgs.put(info.name, info);
                defaultableCount++;
            }
        }

        int requiredParamsCount = hasRestParam ? funcInfo.length - defaultableCount - 1 : funcInfo.length - defaultableCount;
        ...
        if (defaultableCount > 0) {
            ...
            Arrays.fill(bValueArgs, defaultableArgIndex, bValueArgs.length - 1, false);
        }
        ...
        for (String arg : args) {
            if (isDefaultParamCandidate(arg) && namedArgs.containsKey(getParamName(arg))) {
                ...
                bValueArgs[info.index + defaultableArgIndex] = getBValue(info.type, getValueString(arg));
                bValueArgs[info.index + defaultableArgIndex + 1] = true;
            } else {
        ...
        switch (type.getTag()) {
            ...
            case TypeTags.TUPLE_TAG:
                if (!value.startsWith("(") || !value.endsWith(")")) {
                    throw BallerinaErrors.createError("invalid argument '" + value + "', "
                            + "expected tuple notation (\"()\") with tuple arg");
                }
            ...
            case TypeTags.ARRAY_TAG:
                try {
                    ...
                    throw BallerinaErrors.createError("invalid argument '" + value + "', expected array elements of "
    ...
    private static boolean getBooleanValue(String argument) {
        if (!TRUE.equalsIgnoreCase(argument) && !FALSE.equalsIgnoreCase(argument)) {
            throw BallerinaErrors.createError("invalid argument '" + argument + "', expected boolean value 'true' or "
                                                  + "'false'");
        }
    ...
    private static byte getByteValue(String argument) {
        byte byteValue; // TODO: 7/4/18 Allow byte literals?
        try {
            byteValue = Byte.parseByte(argument);
        } catch (NumberFormatException e) {
    ...
    private static ArrayValue parseTupleArg(BTupleType type, String tupleArg) {
        ...
        if (tupleElements.length != type.getTupleTypes().size()) {
            throw BallerinaErrors.createError("invalid argument '(" + tupleArg + ")', element count mismatch for tuple "
                                                  + "type: '" + type + "'");
        }
        ...
        for (BType elementType : type.getTupleTypes()) {
            ...
            try {
                ...
                tupleValues.add(index, (RefValue) getBValue(elementType, tupleElement));
                index++;
            } catch (BallerinaException e) {
                String localizedMessage = e.getLocalizedMessage();
                if (localizedMessage.startsWith(UNSUPPORTED_TYPE_PREFIX)) {
                    throw BallerinaErrors.createError("unsupported element type for tuple as entry function argument: "
                                                          + elementType);
