 ...
 * @since 0.995.0
 */
public class TupleValueImpl extends AbstractArrayValue implements RefValue, BArray {
    ...
    @Deprecated
    public TupleValueImpl(Object[] values, BType type) {
    ...
    @Deprecated
    public TupleValueImpl(long[] values) {
    ...
    @Deprecated
    public TupleValueImpl(boolean[] values) {
    ...
    @Deprecated
    public TupleValueImpl(byte[] values) {
    ...
    @Deprecated
    public TupleValueImpl(double[] values) {
    ...
    @Deprecated
    public TupleValueImpl(String[] values) {
    ...
    @Deprecated
    public TupleValueImpl(BType type) {
    ...
    @Deprecated
    public TupleValueImpl(BType type, long size) {
        this.arrayType = type;
        if (type.getTag() == TypeTags.ARRAY_TAG) {
     ...
     */
    public void unshift(TupleValueImpl values) {
        unshift(0, values);
    }
    ...
    public void unshift(long index, TupleValueImpl vals) {
        handleFrozenArrayValue();
    ...
    private void addToIntArray(TupleValueImpl vals, int startIndex) {
    ...
    private void addToFloatArray(TupleValueImpl vals, int startIndex) {
        int endIndex = startIndex + vals.size;
        for (int i = startIndex, j = 0; i < endIndex; i++, j++) {
    ...
    private void addToStringArray(TupleValueImpl vals, int startIndex) {
        int endIndex = startIndex + vals.size;
        for (int i = startIndex, j = 0; i < endIndex; i++, j++) {
    ...
    private void addToByteArray(TupleValueImpl vals, int startIndex) {
        int endIndex = startIndex + vals.size;
    ...
    private void addToBooleanArray(TupleValueImpl vals, int startIndex) {
        int endIndex = startIndex + vals.size;
        for (int i = startIndex, j = 0; i < endIndex; i++, j++) {
    ...
    private void addToRefArray(TupleValueImpl vals, int startIndex) {
        int endIndex = startIndex + vals.size;
        for (int i = startIndex, j = 0; i < endIndex; i++, j++) {
    ...
    public Object copy(Map<Object, Object> refs) {
        ...
        if (elementType != null) {
            TupleValueImpl valueArray = null;

            if (elementType.getTag() == TypeTags.INT_TAG) {
                valueArray = new TupleValueImpl(Arrays.copyOf(intValues, intValues.length));
            } else if (elementType.getTag() == TypeTags.BOOLEAN_TAG) {
                valueArray = new TupleValueImpl(Arrays.copyOf(booleanValues, booleanValues.length));
            } else if (elementType.getTag() == TypeTags.BYTE_TAG) {
                valueArray = new TupleValueImpl(Arrays.copyOf(byteValues, byteValues.length));
            } else if (elementType.getTag() == TypeTags.FLOAT_TAG) {
                valueArray = new TupleValueImpl(Arrays.copyOf(floatValues, floatValues.length));
            } else if (elementType.getTag() == TypeTags.STRING_TAG) {
                valueArray = new TupleValueImpl(Arrays.copyOf(stringValues, stringValues.length));
            }
        ...
        TupleValueImpl refValueArray = new TupleValueImpl(values, arrayType);
    ...
    public Object frozenCopy(Map<Object, Object> refs) {
        TupleValueImpl copy = (TupleValueImpl) copy(refs);
        if (!copy.isFrozen()) {
            copy.freezeDirect();
        }
        return copy;
    ...
    protected void prepareForAdd(long index, Object value, int currentArraySize) {
        ...
        // check types
        BType elemType = ((BTupleType) this.arrayType).getTupleTypes().get((int) index);
    ...
    static class ArrayIterator implements IteratorValue {
        TupleValueImpl array;
        long cursor = 0;
        long length;

        ArrayIterator(TupleValueImpl value) {
            this.array = value;
