...
public class TypeChecker {
    ...
    public static Object checkCast(Object sourceVal, Type targetType) {
        ...
        // if the source is a numeric value and the target type is a union, try to find
        // a matching
        // member.
     ...
     * @return true if the value has the same shape as the given type; false
     *         otherwise
     */
     ...
     * @param sourceValue            value to check the shape
     * @param targetType             type to check the shape against
     * @param allowNumericConversion whether numeric conversion is allowed to change
     *                               the shape to the target type
     * @return true if the value has the same shape as the given type; false
     *         otherwise
     */
    ...
    /**
     * Reference equality check for values. If both the values are simple basic
     * types, returns the same
     * result as {@link #isEqual(Object, Object, List)}
     *
     * @param lhsValue The value on the left hand side
     * @param rhsValue The value on the right hand side
     * @return True if values are reference equal or in the case of simple basic
     *         types if the values are equal,
     *         else false.
     */
    ...
    private static boolean isXMLSequenceRefEqual(XmlSequence lhsValue, XmlSequence rhsValue) {
        ...
        // only one hasNext method returns true means sequences are of different sizes,
        // hence not ref equal
     ...
     * @param lhsValue      The value on the left hand side
     * @param rhsValue      The value on the right hand side
     * @param checkedValues Structured value pairs already compared or being
     *                      compared
     * @return True if values are equal, else false.
     */
    ...
    private static boolean isEqual(TableValueImpl lhsTable, TableValueImpl rhsTable, List<ValuePair> checkedValues) {
        ...
        boolean isLhsKeyedTable = TypeHelper.tableFieldNames(getImpliedType(lhsTable.getType())).length > 0;
        boolean isRhsKeyedTable = TypeHelper.tableFieldNames(getImpliedType(rhsTable.getType())).length > 0;
