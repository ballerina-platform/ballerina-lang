...
public class WebSocketConnectionManager {
     ...
     * connectors.
     *
     * @param service {@link HttpService} of the service which session is related.
     * @param session Session of the connection.
     * @param carbonMessage carbon message received.
     */
    public synchronized void addServerSession(HttpService service, Session session, CarbonMessage carbonMessage) {
        /*
     ...
     */
    private synchronized void addConnectionToBroadcast(HttpService service, Session session) {
     ...
     *
     * @param parentService {@link HttpService} where the connector is created.
            ...
            for (Session clientSession : serverSessionToClientSessionsMap.remove(session.getId())) {
                if (clientSession.isOpen()) {
                    try {
                        ...
                        throw new BallerinaConnectorException("Internal error occurred when closing client connection");
                    }
                }
            }
        }
    ...
    private Session initializeClientConnection(BConnector bConnector, CarbonMessage carbonMessage) {
        ...
        try {
            ...
            throw new BallerinaConnectorException("Error occurred during initializing the connection to " + remoteUrl);
        }
