...
import log from 'log';
import BallerinaView from './../views/ballerina-view';
import VariablesView from './../views/variables-view';
import TypeStructDefinition from './../views/type-struct-definition-view';
import BallerinaASTFactory from 'ballerina/ast/ballerina-ast-factory';
import TypeMapper from 'typeMapper';
import InputStructView from './../views/input-struct-view';
import OutputStructView from './../views/output-struct-view';
import TypeMapperStatement from './../views/type-mapper-statement-view';
import TypeMapperBlockStatement from './../views/type-mapper-block-statement-view';
...
class TypeMapperDefinition extends React.Component {
    ...
    constructor(props) {
        ...
        this.currentTypeMappers = [];
        this._onConnectInstance = {};
        this._onDisconnectInstance = {};
        this._sourceInfo = {};
        this._targetInfo = {};
        this._selectedSourceStruct = TYPE_MAPPER_COMBOBOX_DEFAULT_SELECTION;
        this._selectedTargetStruct = TYPE_MAPPER_COMBOBOX_DEFAULT_SELECTION;
    }

    render() {
        var self = this;
        var isTypeMapperRendered = false;

        !_.forEach(this.currentTypeMappers, currentTypeMapper => {
            if(currentTypeMapper == self.props.model.getTypeMapperName()) {
                isTypeMapperRendered = true;
            }
        });

        if (!isTypeMapperRendered) {


        //this.props.model

        var resourceParameter = _.find(self.props.model.getChildren(), function (child) {
        ...
        var returnType = _.find(self.props.model.getChildren(), function (child) {
    ...
    //    this.getPanelIcon().addClass(_.get(this._viewOptions, "cssClass.type_mapper_icon", ""));
        ...
        var currentContainer = $('#' + this.props.model.getID());
        ...
        $(this.props.model.getTypeMapperName()).text(this.props.model.getTypeMapperName())
            .on("change paste keyup", function () {
                self.props.model.setTypeMapperName($(this).text());
                ...
                if (!_.isEqual(e.key, "Delete") && !_.isEqual(e.key, "Backspace")) {
                    ...
                    try {
                        self.props.model.setTypeMapperName(newTypeMapperName);
                    } catch (error) {
                        e.stopPropagation();
                        return false;
                    }
        ...
        var dataMapperContainerId = "data-mapper-container___" + this.props.model.id;
        ...
        $(document).ready(function() {
            ...
            self.loadSchemasToComboBox(dataMapperContainerId, "#" + sourceId, "#" + targetId, predefinedStructs);
            ...
            $("#" + targetId).on("select2:open", function () {
                ...
                if (predefinedStructs.length > 0) {
                    ...
                    self.loadSchemaToComboBox(dataMapperContainerId, "#" + targetId, predefinedStructs);
            ...
            $("#" + sourceId).on("select2:open", function () {
                ...
                if (predefinedStructs.length > 0) {
                    ...
                    self.loadSchemaToComboBox(dataMapperContainerId, "#" + sourceId, predefinedStructs);
            ...
            $(currentContainer).find("#" + sourceId).change(function () {
                ...
                if (selectedNewStructNameForSource != TYPE_MAPPER_COMBOBOX_DEFAULT_SELECTION && selectedNewStructNameForSource
                    ...
                    if(selectedNewStructNameForSource == self.getSelectedTargetStruct()){
                        ...
                        self.props.model.removeReturnType();
                        self.setSelectedTargetStruct(TYPE_MAPPER_COMBOBOX_DEFAULT_SELECTION);
                        self.setTargetSchemaNameToComboBox('#targetStructs' + self.props.model.id, self.getSelectedTargetStruct());
                    ...
                    self.props.model.removeResourceParameter();
                    self.props.model.addResourceParameterChild(selectedNewStructNameForSource, "y");
                }else if(selectedNewStructNameForSource == TYPE_MAPPER_COMBOBOX_DEFAULT_SELECTION){
                    self.setSourceSchemaNameToComboBox('#sourceStructs' + self.props.model.id, self.getSelectedSourceStruct());
                }
            });
            ...
            $(currentContainer).find("#" + targetId).change(function () {
                ...
                if (selectedStructNameForTarget != TYPE_MAPPER_COMBOBOX_DEFAULT_SELECTION && selectedStructNameForTarget
                    ...
                    self.props.model.removeReturnType();
                    self.props.model.addReturnTypeChild(selectedStructNameForTarget, "x");
                    self.props.model.fillReturnStatement("x");
                    self.props.model.fillVariableDefStatement(selectedStructNameForTarget, "x");
                }else if(selectedStructNameForTarget == TYPE_MAPPER_COMBOBOX_DEFAULT_SELECTION){
                    self.setTargetSchemaNameToComboBox('#targetStructs' + self.props.model.id, self.getSelectedTargetStruct());
                }
            });

       //     self.props.model.accept(self);
        //        self.props.model.on('child-added', function (child) {
         //       self.visit(child);
          //  });
            ...
            var dropActiveClass = _.get(self._viewOptions, 'cssClass.design_view_drop');
                ...
                if (self.toolPalette.dragDropManager.isOnDrag()) {
                    ...
                    self.toolPalette.dragDropManager.setActivatedDropTarget(self.props.model.getBlockStatement(), function (nodeBeingDragged) {
                        ...
                        function (nodeBeingDragged) {
                            if (BallerinaASTFactory.isAssignmentStatement(nodeBeingDragged)) {
                                ...
                                return _.findLastIndex(self.props.model.getBlockStatement().getChildren());
                            } else if (BallerinaASTFactory.isFunctionInvocationStatement(nodeBeingDragged)) {
                                var functionInvocationExp = nodeBeingDragged.getChildren()[0];
                                var functionSchema = self.getFunctionSchema(functionInvocationExp, self.getDiagramRenderingContext());
                                _.forEach(functionSchema.parameters, function (params) {
                ...
                if (self.toolPalette.dragDropManager.isOnDrag()) {
                    if (_.isEqual(self.toolPalette.dragDropManager.getActivatedDropTarget(), self.props.model.getBlockStatement())) {
                ...
                self.props.model.on('after-remove', function (child) {
                    self.stopListening();
            },self);
         ...
         var typeMapper = $('<div id="tp"></div>');
         ReactDOM.render(<Hello name="World" />, typeMapper.get(0));
         this.context.container[0].append(typeMapper.get(0));

         typeMapper.get(0).append(dataMapperContainer.get(0));
         this.currentTypeMappers.push(this.props.model.getTypeMapperName());
...
TypeMapperDefinition.contextTypes = {
