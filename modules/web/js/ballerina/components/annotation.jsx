...
class Annotation extends React.Component {
    ...
    render() {
        ...
        let addIcon = (null);
        let attributesOfCurrentAnnotation = this.getAnnotationAttributes(
        ...
        let annotationAttributeComponents = [];
        for (let annotationAttribute of model.getChildren()) {
            annotationAttributeComponents.push(<tr key={annotationAttribute.getID()} className='annotation-attribute-row' >
                <td></td>
                <td>
                    <table>
                        <tbody>
                            <AnnotationAttribute model={annotationAttribute}
            ...
            for (let annotationDefinition of packageDefintion.getAnnotationDefinitions()) {
                if (annotationDefinition.getAttachmentPoints().length === 0 ||
                    ...
                    if (this.props.model.getParent()) {
                        let parentNodeAnnotations = this.props.model.getParent().getChildrenOfType(ASTFactory.isAnnotation);
    ...
    onPackageNameSelected(event, { suggestionValue }) {
        ...
        if (this.props.model.getFullPackageName() !== suggestionValue) {
            ...
            for (let annotationAttribute of this.props.model.getChildren()) {
            ...
            if (packageDefintion.getName() === fullPackageName) {
                for (let annotationDefinition of packageDefintion.getAnnotationDefinitions()) {
                    if (annotationDefinition.getName() === identifier) {
                        for (let annotationAttribute of annotationDefinition.getAnnotationAttributeDefinitions()) {
