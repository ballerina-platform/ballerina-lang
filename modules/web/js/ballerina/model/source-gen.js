    ...
    switch (node.kind) {
        ...
        case 'Assignment':
            if (node.declaredWithVar && node.variables && node.expression) {
                return dent() + w() + 'var'
                 + join(node.variables, pretty, l, w, '', ',') + w() + '=' + getSourceOf(node.expression, pretty, l) + w()
                 + ';';
            } else {
        ...
        case 'Connector':
            if (node.name.value && node.variableDefs && node.actions) {
                return dent() + w() + 'connector' + w() + node.name.value + w() + '('
                 + w() + ')' + w() + '{' + indent()
                 + join(node.variableDefs, pretty, l, w, '') + join(node.actions, pretty, l, w, '') + outdent()
                 + w() + '}';
            } else {
                return dent() + w() + 'connector' + w() + node.name.value + w() + '('
                 + w() + ')' + w() + '{' + indent()
        ...
        case 'ConnectorInitExpr':
            if (node.connectorType && node.expressions) {
                return w() + 'create' + getSourceOf(node.connectorType, pretty, l) + w()
        ...
        case 'If':
            if (node.condition && node.body && node.elseStatement
                ...
                return (node.parent.kind === 'If' ? '' : dent()) + w() + 'if' + w()
                 + '(' + getSourceOf(node.condition, pretty, l) + w() + ')' + w()
                 + '{' + indent() + getSourceOf(node.body, pretty, l) + outdent()
                 + w() + '}' + w() + 'else'
                ...
                return (node.parent.kind === 'If' ? '' : dent())
                 ...
                 + getSourceOf(node.body, pretty, l) + outdent() + w() + '}' + w()
                 + 'else' + w() + '{' + indent()
        ...
        case 'Resource':
            return dent() + join(node.annotationAttachments, pretty, l, w, '')
                 + join(node.parameters, pretty, l, w, '', ',') + w() + '{' + indent()
        ...
        case 'Throw':
            return dent() + w() + 'throw' + getSourceOf(node.expressions, pretty, l)
                 + w() + ';';
        case 'Transform':
            return getSourceOf(node.body, pretty, l);
        case 'TypeCastExpr':
        ...
        case 'Variable':
            ...
            } else if (node.typeNode && node.initialExpression && node.name.value) {
                return dent() + getSourceOf(node.typeNode, pretty, l)
                 + getSourceOf(node.initialExpression, pretty, l) + w() + 'int' + w()
                 + node.name.value + w() + '=' + w() + ';';
            } else if (node.typeNode && node.name.value) {
                return getSourceOf(node.typeNode, pretty, l) + w() + node.name.value;
            } else if (node.initialExpression && node.name.value) {
                return getSourceOf(node.initialExpression, pretty, l) + w()
                 + node.name.value + w() + '=';
            } else {
        ...
        case 'While':
            return dent() + w() + 'while' + w() + '('
                 ...
                 + getSourceOf(node.body, pretty, l) + outdent() + w() + '}';
        case 'Worker':
            return dent() + w() + '{' + indent() + getSourceOf(node.body, pretty, l)
                 + outdent() + w() + '}';
        case 'WorkerReceive':
