...
function populateInnerPanelDecoratorBBoxPosition(node) {
    ...
    if (currentResourceIndex === 0) {
         ...
         */
        node.getParent().filterChildren(child => ASTFactory.isConnectorDeclaration(child));
...
function populateOuterPanelDecoratorBBoxPosition(node) {
    ...
    resources.forEach((element) => {
        const resourceViewState = element.getViewState();
        // if the service width is wider than resource width we will readjust.
        if (resourceViewState.bBox.w + connectorWidthTotal < minWidth) {
            resourceViewState.bBox.w = minWidth - connectorWidthTotal;
        }
    }, this);
}
...
class PositioningUtil {
    ...
    positionFunctionNode(node) {
        ...
        cmp.statementContainer.x = viewState.bBox.x + DesignerDefaults.panel.body.padding.left;
        cmp.statementContainer.y = viewState.bBox.y + cmp.heading.h + DesignerDefaults.panel.body.padding.top +
                                                                                DesignerDefaults.lifeLine.head.height;

        cmp.defaultWorker.x = cmp.statementContainer.x +
                                                    ((cmp.statementContainer.w - DesignerDefaults.lifeLine.width) / 2);
        cmp.defaultWorker.y = cmp.statementContainer.y - DesignerDefaults.lifeLine.head.height;

        // position the children
        const body = node.getBody();
        body.viewState.bBox.x = viewState.bBox.x + DesignerDefaults.panel.body.padding.left;
        body.viewState.bBox.y = viewState.bBox.y + cmp.heading.h + DesignerDefaults.panel.body.padding.top +
                                                                                DesignerDefaults.lifeLine.head.height;
    }
