...
class WorkerReplyStatementDimensionCalculatorVisitor {
    ...
    endVisit(node) {
        ...
        if (!_.isUndefined(workerDeclaration)) {
            const heightFromTop = this.sizingUtil.getStatementHeightBefore(node);
            ...
            if (!_.isUndefined(workerReplyStatement)) {
                const workerHeightFromTop = this.sizingUtil.getStatementHeightBefore(workerReplyStatement);
