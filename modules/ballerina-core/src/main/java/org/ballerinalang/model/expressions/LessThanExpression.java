...
public class LessThanExpression extends BinaryCompareExpression {
    ...
    public static final BiFunction<BValue, BValue, BValue> LESS_THAN_INT_FUNC =
            (lVal, rVal) -> new BBoolean(((BValueType) lVal).intValue() < ((BValueType) rVal).intValue());

    public static final BiFunction<BValue, BValue, BValue> LESS_THAN_FLOAT_FUNC =
            (lVal, rVal) -> new BBoolean(((BValueType) lVal).floatValue() < ((BValueType) rVal).floatValue());

    public LessThanExpression(NodeLocation location, WhiteSpaceDescriptor whiteSpaceDescriptor, Expression lExpr,
                              Expression rExpr) {
        super(location, whiteSpaceDescriptor, lExpr, LESS_THAN, rExpr);
    }
