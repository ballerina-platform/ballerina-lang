...
public class JMSListenerImpl implements JMSListener {
    ...
    public void onMessage(CarbonMessage carbonMessage, JMSCallback jmsCallback) {
        ...
        try {
            // Find the Service
            ServiceInfo service = dispatcher.findService(carbonMessage, jmsCallback);
            if (service == null) {
            ...
            // Find the Resource
            ResourceInfo resource = resourceDispatcher.findResource(service, carbonMessage, jmsCallback);
            invokeResource(carbonMessage, jmsCallback, protocol, resource, service);
        } catch (Throwable throwable) {
            handleError(carbonMessage, jmsCallback, throwable);
        }
    }
