...
@BallerinaFunction(
        ...
        args = {@Argument(name = "actual", type = TypeEnum.STRING),
                @Argument(name = "expected", type = TypeEnum.STRING),
                @Argument(name = "message", type = TypeEnum.STRING) },
        isPublic = true
)
public class AssertEqualsString extends AbstractNativeFunction {

    @Override
    public BValue[] execute(Context context) {
        String actual = getArgument(context, 0).stringValue();
        String expected = getArgument(context, 1).stringValue();
