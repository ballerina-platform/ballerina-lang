...
import org.wso2.ballerina.core.nativeimpl.connectors.jms.utils.JMSMessageUtils;
...
public class Send extends AbstractJMSAction {
    ...
    public BValue execute(
        ...
        if (messageType.equalsIgnoreCase(JMSConstants.TEXT_MESSAGE_TYPE) ||
            ...
            if (bMessage.getMessageDataSource() instanceof StringDataSource) {
                message = new TextCarbonMessage(
                        ((StringDataSource) bMessage.getMessageDataSource()).getValue());
            }
        ...
        if (properties.get(new BString(JMSConstants.CONNECTION_PASSWORD)) != null) {
            propertyMap.put(JMSConstants.CONNECTION_PASSWORD,
                            properties.get(new BString(JMSConstants.CONNECTION_PASSWORD))
                                      .stringValue());
        }
