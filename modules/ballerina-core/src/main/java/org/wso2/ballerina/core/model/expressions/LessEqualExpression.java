...
public class LessEqualExpression extends BinaryCompareExpression {
    ...
    public static final BiFunction<BValueRef, BValueRef, BValueRef> LESS_EQUAL_INT_FUNC =
            (lVal, rVal) -> {
                BValue resultVal = new BooleanValue(lVal.getInt() <= rVal.getInt());
                return new BValueRef(resultVal);
            };

    public static final BiFunction<BValueRef, BValueRef, BValueRef> LESS_EQUAL_LONG_FUNC =
            (lVal, rVal) -> {
                BValue resultVal = new BooleanValue(lVal.getLong() <= rVal.getLong());
                return new BValueRef(resultVal);
            };

    public static final BiFunction<BValueRef, BValueRef, BValueRef> LESS_EQUAL_FLOAT_FUNC =
            (lVal, rVal) -> {
                BValue resultVal = new BooleanValue(lVal.getFloat() <= rVal.getFloat());
                return new BValueRef(resultVal);
            };

    public static final BiFunction<BValueRef, BValueRef, BValueRef> LESS_EQUAL_DOUBLE_FUNC =
            (lVal, rVal) -> {
