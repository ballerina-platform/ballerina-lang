...
@BallerinaFunction(
        ...
        args = {@Argument(name = "actual", type = TypeEnum.BOOLEAN),
                @Argument(name = "expected", type = TypeEnum.BOOLEAN),
                @Argument(name = "message", type = TypeEnum.STRING) },
        isPublic = true
)
public class AssertEqualsBoolean extends AbstractNativeFunction {

    @Override
    public BValue[] execute(Context context) {
        BBoolean actual = ((BBoolean) getArgument(context, 0));
        BBoolean expected = ((BBoolean) getArgument(context, 1));
