...
public class StatementVisitor extends BallerinaBaseVisitor {
    ...
    public Object visitStatement(BallerinaParser.StatementContext ctx) {
        if (ctx.assignmentStatement() != null) {
            AssignmentStatementVisitor asv = new AssignmentStatementVisitor();
            AssignStmt assignStmt = (AssignStmt) ctx.assignmentStatement().accept(asv);
            return assignStmt;
        }
        // Need to do the same for different statement types
