 ...
 * ballerina.model.datatables:getFloat(datatable, any)
 *
 * @since 0.88
 */
@BallerinaFunction(
...
public class GetFloat extends AbstractNativeFunction {
    ...
    public BValue[] execute(Context ctx) {
        ...
        if (index instanceof BInteger) {
            result = getBValues(new BFloat(dataTable.getFloat(((BInteger) index).intValue())));
        } else if (index instanceof BString) {
            result = getBValues(new BFloat(dataTable.getFloat(((BString) index).stringValue())));
        }
        return result;
    }
}
