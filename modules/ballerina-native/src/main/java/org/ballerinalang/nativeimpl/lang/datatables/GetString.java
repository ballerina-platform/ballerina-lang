 ...
 * ballerina.model.datatables:getString(datatable, any)
 *
 * @since 0.88
 */
@BallerinaFunction(
...
public class GetString extends AbstractNativeFunction {
    ...
    public BValue[] execute(Context ctx) {
        ...
        if (index instanceof BInteger) {
            result = getBValues(new BString(dataTable.getString(((BInteger) index).intValue())));
        } else if (index instanceof BString) {
            result = getBValues(new BString(dataTable.getString(((BString) index).stringValue())));
        }
        return result;
    }
}
