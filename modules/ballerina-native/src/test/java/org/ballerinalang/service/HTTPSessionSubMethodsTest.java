...
public class HTTPSessionSubMethodsTest {
    ...
    @Test(description = "Test for manual session invalidation")
    public void testManualSessionInvalidation() throws InterruptedException {
        CarbonMessage cMsg = MessageUtils.generateHTTPMessage("/sample2/new9", "GET");
        CarbonMessage response = Services.invoke(cMsg);
        Assert.assertNotNull(response);
        ...
        Assert.assertNotNull(stringDataSource);
        long timeInterval2 = Long.parseLong(stringDataSource.getValue().toString());
        Assert.assertNotEquals(cookie, response.getHeader(RESPONSE_COOKIE_HEADER));
        Assert.assertEquals(timeInterval2, 900);
    }

    @Test(description = "Test for manual session invalidation in two resources")
    public void testManualSessionInvalidationTwoResources() throws InterruptedException {
        CarbonMessage cMsg = MessageUtils.generateHTTPMessage("/sample2/new9", "GET");
        CarbonMessage response = Services.invoke(cMsg);
        Assert.assertNotNull(response);
    ...
    @Test(description = "Test for manual session invalidation in getSession")
    public void testManualSessionInvalidationInGetSession() throws InterruptedException {
        CarbonMessage cMsg = MessageUtils.generateHTTPMessage("/sample2/new9", "GET");
        CarbonMessage response = Services.invoke(cMsg);
        Assert.assertNotNull(response);
        ...
        String sessionId = cookie.substring(SESSION_ID.length(), cookie.length() - 14);
        Thread.sleep(1000);
        cMsg = MessageUtils.generateHTTPMessage("/sample2/id2", "GET");
        cMsg.setHeader(COOKIE_HEADER, SESSION_ID + sessionId);
        response = Services.invoke(cMsg);
        Assert.assertNotNull(response);
