...
public class ConstructProviderClassBuilder {
    ...
    private final String importPkg = "import " + NativeScope.class.getCanonicalName() + ";\n" +
                                     ...
                                     "import " + BuiltinTypeName.class.getCanonicalName() + ";\n" +
                                     "import " + UserDefinedTypeName.class.getCanonicalName() + ";\n" +
    ...
    private String getReturnTypes(ReturnType[] returnTypes) {
        ...
        String builtinTypeNameClass = BuiltinTypeName.class.getSimpleName();
        String userDefinedTypeNameClass = UserDefinedTypeName.class.getSimpleName();
        ...
        for (ReturnType returnType : returnTypes) {
            ...
            if (returnType.type().equals(TypeEnum.ARRAY)) {
                isArrayType = true;
                typeName = returnType.elementType().getName();
                dimensions = returnType.arrayDimensions();
            } else {
                typeName = returnType.type().getName();
            }

            if (returnType.type().equals(TypeEnum.STRUCT)) {
                if (isArrayType) {
                    sb.append("new " + userDefinedTypeNameClass + "(null, \"" + returnType.structType() + "\", \""
                            + returnType.structPackage() + "\", " + dimensions + ")");
                } else {
                    sb.append("new " + userDefinedTypeNameClass + "(null, \"" + returnType.structType() + "\", \""
                            + returnType.structPackage() + "\")");
                }
            } else {
                if (isArrayType) {
                    sb.append("new " + builtinTypeNameClass + "(\"" + typeName + "\", " + dimensions + ")");
                } else {
                    sb.append("new " + builtinTypeNameClass + "(\"" + typeName + "\")");
                }
            }
     ...
     * @param enclosingScopeName scope name
     * @param enclosingScopePkg scope package name
    ...
    private String getArgTypes(Argument[] arguments, String enclosingScopeName, String enclosingScopePkg) {
        ...
        for (Argument argType : arguments) {
            ...
            if (argType.type().equals(TypeEnum.ARRAY)) {
                ...
                dimensions = argType.arrayDimensions();
            } else {
                bType = argType.type();
            }
            ...
            if (bType == TypeEnum.CONNECTOR) {
                if (isArrayType) {
                    sb.append("new " + userDefinedTypeNameClass + "(null, \"" + enclosingScopeName + "\",\"" +
                            enclosingScopePkg + "\", " + dimensions + ")");
                } else {
                    sb.append("new " + userDefinedTypeNameClass + "(null, \"" + enclosingScopeName + "\",\"" +
                            enclosingScopePkg + "\")");
                }
                ...
                if (isArrayType) {
                    sb.append("new " + userDefinedTypeNameClass + "(null, \"" + argType.structType() + "\",\""
                            + pkg + "\", " + dimensions + ")");
                } else {
                    sb.append("new " + userDefinedTypeNameClass + "(null, \"" + argType.structType() + "\",\""
                            + pkg + "\")");
                ...
                if (isArrayType) {
                    sb.append("new " + builtinTypeNameClass + "(null, \"" + bType.getName() +
                            "\", " + dimensions + ")");
                } else {
                    sb.append("new " + builtinTypeNameClass + "(null, \"" + bType.getName() + "\")");
                }
