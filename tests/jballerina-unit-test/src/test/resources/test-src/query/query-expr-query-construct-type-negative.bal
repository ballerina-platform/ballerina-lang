...
function testQueryConstructWithInvalidStaticType() {
    var _ = from string letter in "abcd" select 1; //error: incompatible types: 'string' cannot be constrained with 'int'
    var _ = from string letter in "abcd" select [1, 2, 3, 4]; //error: incompatible types: 'string' cannot be constrained with '[int,int,int,int]'

    xml x = xml `<book><a>The Lost World</a><b>Clean Code</b></book>`;

    var _ = from xml element in x select element.toString(); //error: incompatible types: 'xml' cannot be constrained with 'string'
    var _ = from xml element in x select 1; //error: incompatible types: 'xml' cannot be constrained with 'int'
    var _ = from xml element in x select [1, 2, 3, 4]; //error: incompatible types: 'xml' cannot be constrained with '[int,int,int,int]'
    ...
    var _ = from Employee e in t select 1; //error: incompatible types: 'table<Employee> key(name)' cannot be constrained with 'int'
    var _ = from Employee e in t select ["name", 35000]; //error: incompatible types: 'table<Employee> key(name)' cannot be constrained with '[string,int]'
    var _ = from Employee e in t select t; //error: incompatible types: 'table<Employee> key(name)' cannot be constrained with 'table<Employee> key(name)'

    map<int> m = {"a": 1, "b": 2};

    var _ = from var i in m select ["A",1]; //error: query expression that constructs a mapping must start with the map keyword
    var _ = from var i in m select 1; //error: query expression that constructs a mapping must start with the map keyword
    var _ = from var i in m select {"A": 1}; //error: query expression that constructs a mapping must start with the map keyword
    var _ = map from var i in m select 1; //error: incompatible type in select clause: expected [string,any|error], found 'int'
    var _ = map from var i in m select {"A": 1}; //error: incompatible type in select clause: expected [string,any|error], found 'record {| int A; |}'
    ...
    var _ = from var i in t2 select 1; //error: incompatible types: 'table<Employee> key(name)' cannot be constrained with 'int'
    var _ = from var i in t2 select t2; //error: incompatible types: 'table<Employee> key(name)' cannot be constrained with 'T1'
}
