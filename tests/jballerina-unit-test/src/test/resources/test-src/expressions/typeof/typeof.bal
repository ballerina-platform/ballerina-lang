...
function typeDescOfExpressionsOfLiterals() {
    ...
    int[] a = [1, 2, 3];
    test:assertEquals((typeof a).toString(), "typedesc int[]");

    [string, int] t = ["Hello", 123];
    test:assertEquals((typeof t).toString(), "typedesc [string,int]");

    string result = "Hello World";
    foreach var c in result {
        test:assertEquals((typeof c).toString(), "typedesc H");
        break;
    }
    test:assertEquals((typeof result.iterator()).toString(), "typedesc lang.string:StringIterator");
    future<int> futureResult = start foo();
    test:assertEquals((typeof futureResult).toString(), "typedesc future<int>");

    map<string> m = {"name": "Waruna"};
    test:assertEquals((typeof m).toString(), "typedesc map<string>");

    var regExpr = re `World`;
    test:assertEquals((typeof regExpr).toString(), "typedesc World");

    foreach var iterator in m.entries() {
        test:assertEquals((typeof iterator).toString(), "typedesc [string,string]");
    }
    test:assertEquals((typeof m.iterator()).toString(), "typedesc lang.map:MapIterator");
    table<Person> key(name) personTable = table [
        {name: "Hinduja"},
        {name: "Gabilan"},
        {name: "Nadeeshan"}
    ];
    test:assertEquals((typeof personTable).toString(), "typedesc table<Person> key(name)");
    test:assertEquals((typeof personTable.iterator()).toString(), "typedesc lang.table:TableIterator");
    xml x1 = xml `<book>The Lost World</book>`;
    test:assertEquals((typeof x1).toString(), "typedesc lang.xml:Element");
    test:assertEquals((typeof x1.iterator()).toString(), "typedesc lang.xml:XMLIterator");

    xml x2 = xml `<?target data?>`;
    test:assertEquals((typeof x2).toString(), "typedesc lang.xml:ProcessingInstruction");
    test:assertEquals((typeof x2.iterator()).toString(), "typedesc lang.xml:XMLIterator");

    xml x3 = xml `<!--I am a comment-->`;
    test:assertEquals((typeof x3).toString(), "typedesc lang.xml:Comment");
    test:assertEquals((typeof x3.iterator()).toString(), "typedesc lang.xml:XMLIterator");

    xml x4 = xml `Hello, world!`;
    test:assertEquals((typeof x4).toString(), "typedesc Hello, world!");
    test:assertEquals((typeof x4.iterator()).toString(), "typedesc lang.xml:XMLIterator");

    Engineer engineer = new Engineer("Alice");
    test:assertEquals((typeof engineer).toString(), "typedesc Engineer");
}
