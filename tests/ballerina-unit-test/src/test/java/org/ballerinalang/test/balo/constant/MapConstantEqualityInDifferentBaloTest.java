...
public class MapConstantEqualityInDifferentBaloTest {
    ...
    public void setup() {
        ...
        compileResult = BCompileUtil.compile("test-src/balo/test_balo/constant/map-literal-constant-equality" +
                "-different-modules.bal");
    }
    ...
    public void testComplexNilMapReferenceEqualityUsingDifferentReference() {
        BValue[] returns = BRunUtil.invoke(compileResult, "testComplexNilMapReferenceEqualityUsingDifferentReference");
        Assert.assertNotNull(returns[0]);
        Assert.assertFalse(((BBoolean) returns[0]).booleanValue());
    }
}
