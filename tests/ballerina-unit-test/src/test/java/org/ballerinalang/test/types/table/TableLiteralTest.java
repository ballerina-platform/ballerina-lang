...
public class TableLiteralTest {
    ...
    public void testPrintData() throws IOException {
        ...
        try {
            ...
            Assert.assertEquals(outContent.toString().replaceAll(CARRIAGE_RETURN_CHAR, EMPTY_STRING),
                    "table<Person> {index: [], primaryKey: [\"id\", \"age\"], : [{id:1, age:30, "
    ...
    @Test(priority = 1,
          description = "Test add  with  mismatched types")
    ...
    @Test(priority = 1,
          description = "Test remove  with mismatched record types")
    ...
    @Test(priority = 1,
          description = "Test remove  with mismatched input parameter types")
