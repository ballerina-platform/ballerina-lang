...
public class MainFunctionsTest {
    ...
    public void testNilReturningMain() throws IOException {
        ...
        Assert.assertEquals(tempOutStream.toString(), "nil returning main invoked",
                            "expected the main function to be invoked");
        Assert.assertEquals(result.length, 1, "expected the main function to return a single value");
        Assert.assertTrue(result[0] == null, "expected nil to be returned");
    }
    ...
    public void testErrorReturningMain() throws IOException {
        ...
        resetTempOut();
        BValue[] result = BLangProgramRunner.runMainFunc(programFile, new String[]{});
        Assert.assertEquals(tempOutStream.toString(), "error returning main invoked",
                            "expected the main function to be invoked");
        Assert.assertEquals(result.length, 1, "expected the main function to return a single value");
        Assert.assertTrue(result[0] instanceof BError, "expected error to be returned");
        Assert.assertEquals(((BError) result[0]).reason, "error return", "invalid error reason");
    }
    ...
    public void testErrorOrNilReturningMainReturningError() throws IOException {
        ...
        resetTempOut();
        BValue[] result = BLangProgramRunner.runMainFunc(programFile, new String[]{"error"});
        Assert.assertEquals(tempOutStream.toString(), "error? returning main invoked",
                            "expected the main function to be invoked");
        Assert.assertTrue(result[0] instanceof BError, "expected error to be returned");
        Assert.assertEquals(((BError) result[0]).reason, "generic error", "invalid error reason");
    }
    ...
    public void testErrorOrNilReturningMainReturningNil() throws IOException {
        programFile = LauncherUtils.compile(Paths.get(MAIN_FUNCTION_TEST_SRC_DIR),
                                            Paths.get ("test_main_with_error_or_nil_return.bal"), false, true);
        resetTempOut();
        BValue[] result = BLangProgramRunner.runMainFunc(programFile, new String[]{"nil"});
        Assert.assertEquals(tempOutStream.toString(), "error? returning main invoked",
                            "expected the main function to be invoked");
        Assert.assertTrue(result[0] == null, "expected nil to be returned");
    }
    ...
    public void testErrorOrNilReturningMainReturningCustomError() throws IOException {
        programFile = LauncherUtils.compile(Paths.get(MAIN_FUNCTION_TEST_SRC_DIR),
                                            Paths.get ("test_main_with_error_or_nil_return.bal"), false, true);
        resetTempOut();
        BValue[] result = BLangProgramRunner.runMainFunc(programFile, new String[]{"user_def_error"});
        Assert.assertEquals(tempOutStream.toString(), "error? returning main invoked",
                            "expected the main function to be invoked");
        Assert.assertTrue(result[0] instanceof BError, "expected error to be returned");
        Assert.assertEquals(((BError) result[0]).reason, "const error reason", "invalid error reason");
        Assert.assertEquals(((BString) ((BMap) ((BError) result[0]).details).get("message")).stringValue(),
