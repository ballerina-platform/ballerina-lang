int count;
string word;

function test1(){
    int x = 0;
    x.foreach(function (int i) returns { count = count + i;});
    string y = "foo";
    y.map(function (string s) returns (int) { return lengthof s;});
}

function test2(){
    string[] y = ["1", "a"];

    y.count();

    y.filter(function (int i, string x) returns (boolean) {
        return true;})
     .foreach(function (string x) returns { word = x;}).count();
}

function test3(){
    map<string> z = {a:"1", b:"2"};
    string[] keys = z.map(
                     function (string s) returns (string, string) {
                         return (s, "value");
    }).keys();
}

function test4() {
    map z = {a:"1", b:"2"};
    string[] a = z.map(function (any x) returns (string, string) {
                           var s = <string>x;
                           return (s, "value");
                       });
    map m = z.filter(function (string s) returns boolean {
          return s == null;
    });
    any x = z.filter(function (string s) returns boolean {
         return s == null;
     });
}

function test5(){
    string[] s = ["1", "a"];
    int x;
    x = s.foreach(function (string s) returns {word = word + s;});

    var (z, y) = s.map(function ((int, string) tuple) returns (int, string) { var (i, v) = tuple;
                           return (i * 2, v + v);
                       });
}

function test6(){
    string[] s = ["1", "a"];
    _ = s.count(test5);
    s.filter();
    int i = 10;
    s.foreach(i);
}

function test7(){
    string[] s = ["foo", "bar"];
    s.foreach(function ((string, string, string) z) returns {});
    s.foreach(function () returns {});
    s.filter(function (string s) returns (boolean, int) {return (true, 1);});
    s.filter(function (string s) returns {});
    s.filter(function (person p) returns {});
    _ = s.filter(function (string s) returns (person) {return null;});
}
