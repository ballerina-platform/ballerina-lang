...
function testFilterQuery(stream<Employee> employeeStream, stream<Teacher> teacherStream) {
    forever {
        from teacherStream
        where age > 30
...
function startFilterQuery() returns (Employee[]) {
    ...
    employeeStream5.subscribe(printEmployeeNumber);

    teacherStream8.publish(t1);
