...
service on new http:Listener(21035) {
    ...
    resource function onOpen(http:WebSocketCaller wsEp) {
        http:WebSocketFailoverClient wsClientEp = new({callbackService: failoverretryCallbackService,
        targetUrls: [ "ws://localhost:15300/websocket", "ws://localhost:15200/websocket"], retryConfig: {},
...
service failoverretryCallbackService = @http:WebSocketServiceConfig {} service {
