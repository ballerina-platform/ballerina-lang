    ...
    *grpc:AbstractClientEndpoint;
    ...
    public remote function registerPatient(Patient req, grpc:ClientContext? context = ()) returns ([boolean, grpc:Headers]|grpc:Error) {
        ...
        if context is grpc:ClientContext {
            payload = check self.grpcClient->blockingExecute("HealthServer/registerPatient", req, context.getContextHeaders());
        } else {
            payload = check self.grpcClient->blockingExecute("HealthServer/registerPatient", req, ());
        }

        if payload is [anydata, grpc:Headers] {
            return [<boolean>(payload[0]), payload[1]];
        } else {
    ...
    public remote function getPatientInfo(string req, grpc:ClientContext? context = ()) returns ([Patient, grpc:Headers]|grpc:Error) {
        ...
        [anydata, grpc:Headers]|error payload;
        if context is grpc:ClientContext {
            payload = check self.grpcClient->blockingExecute("HealthServer/getPatientInfo", req, context.getContextHeaders());
        } else {
            payload = check self.grpcClient->blockingExecute("HealthServer/getPatientInfo", req, ());
        }

        if payload is [anydata, grpc:Headers] {
            return [<Patient>(payload[0]), payload[1]];
        } else {
            return <grpc:Error>payload;
        }
    }
...
public type HealthServerClient client object {
    ...
    *grpc:AbstractClientEndpoint;
    ...
    public remote function registerPatient(Patient req, service msgListener, grpc:ClientContext? context = ()) returns (grpc:Error?) {
        ...
        if context is grpc:ClientContext {
            return self.grpcClient->nonBlockingExecute("HealthServer/registerPatient", req, msgListener, context.getContextHeaders());
        }
        return self.grpcClient->nonBlockingExecute("HealthServer/registerPatient", req, msgListener, ());
    }

    public remote function getPatientInfo(string req, service msgListener, grpc:ClientContext? context = ()) returns (grpc:Error?) {

        if context is grpc:ClientContext {
            return self.grpcClient->nonBlockingExecute("HealthServer/getPatientInfo", req, msgListener, context.getContextHeaders());
        }
        return self.grpcClient->nonBlockingExecute("HealthServer/getPatientInfo", req, msgListener, ());
    }
