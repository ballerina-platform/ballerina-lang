 ...
 * under the License.
 */
package org.ballerinalang.test.bala;
...
public class BalaCreator {
    ...
    private static void create(Path projectPath, String packageId, String orgName, String version) throws IOException {
        ...
        Path balaPath = Paths.get(USER_REPO_DEFAULT_DIRNAME);
        ...
        // another level down
        BFileUtil.delete(projectPath.resolve(balaPath).resolve(DOT_BALLERINA_REPO_DIR_NAME));

        // compile and create the bala
        CompileResult compileResult = compileWithTestsAndWrite(projectPath, packageId, buildFolder +
                "/" + BALLERINA_HOME_LIB + "/");

        // copy the bala to the temp-ballerina-home/libs/
        BFileUtil.delete(Paths.get(buildFolder, BALLERINA_HOME_LIB, DOT_BALLERINA_REPO_DIR_NAME, orgName, packageId));
        BFileUtil.copy(projectPath.resolve(balaPath), Paths.get(buildFolder, BALLERINA_HOME_LIB));
    ...
    /**
     * Helper method to create bala from the source and setup the repository.
     *
     * @param projectRoot   root folder location.
     * @param orgName       org name.
     * @param pkgName       package name.
     */
    public static void createAndSetupBala(String projectRoot, String orgName, String pkgName) {
        try {
     ...
     */
    public static void createAndSetupBala(String projectRoot, String orgName, String pkgName, String version) {
        try {
