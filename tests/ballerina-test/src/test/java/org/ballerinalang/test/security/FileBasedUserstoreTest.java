...
public class FileBasedUserstoreTest {
    ...
    public void setup() throws Exception {
        compileResult = BCompileUtil.compile("test-src/security/file-based-userstore.bal");
    ...
    @Test
    public void testReadPasswordHashForNonExistingUser () {
        BValue[] returns = BRunUtil.invoke(compileResult, "testReadPasswordHashForNonExistingUser");
        Assert.assertTrue(returns != null);
        Assert.assertNull(returns[0].stringValue());
    }

    @Test
    public void testReadPasswordHash () {
        BValue[] returns = BRunUtil.invoke(compileResult, "testReadPasswordHash");
        Assert.assertTrue(returns != null);
        BString passwordHash = (BString) returns[0];
        log.info("Password hash: " + passwordHash);
        Assert.assertNotNull(passwordHash);
        Assert.assertEquals(passwordHash.stringValue(),
