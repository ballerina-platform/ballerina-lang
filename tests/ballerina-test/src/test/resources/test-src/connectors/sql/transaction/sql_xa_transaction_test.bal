...
function testXAransactonSuccess() returns (int, int) {
    ...
    var temp = testDB1->select("Select COUNT(*) as countval from Customers where customerId = 1 ", ResultCount, false);
    table dt = check temp;
    while (dt.hasNext()) {
    ...
    temp = testDB2->select("Select COUNT(*) as countval from Salary where id = 1", ResultCount, false);
    dt = check temp;
    while (dt.hasNext()) {
        ResultCount rs = check <ResultCount>dt.getNext();
        count2 = rs.COUNTVAL;
...
function testXAransactonFailed1() returns (int, int) {
    ...
    int count2;
    //check whether update action is performed
    var temp = testDB1->select("Select COUNT(*) as countval from Customers where customerId = 2", ResultCount, false);
    table dt = check temp;
    while (dt.hasNext()) {
        ResultCount rs = check <ResultCount>dt.getNext();
        count1 = rs.COUNTVAL;
    ...
    temp = testDB2->select("Select COUNT(*) as countval from Salary where id = 2 ", ResultCount, false);
    dt = check temp;
    while (dt.hasNext()) {
        ResultCount rs = check <ResultCount>dt.getNext();
        count2 = rs.COUNTVAL;
...
function testXAransactonRetry() returns (int, int) {
    ...
    temp = testDB2->select("Select COUNT(*) as countval from Salary where id = 4", ResultCount, false);
    dt = check temp;
    while (dt.hasNext()) {
        ResultCount rs = check <ResultCount>dt.getNext();
        count2 = rs.COUNTVAL;
