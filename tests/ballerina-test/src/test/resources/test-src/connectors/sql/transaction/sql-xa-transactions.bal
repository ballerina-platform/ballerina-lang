...
function testXAransactonSuccess () returns (int, int) {
    ...
    var temp = testDB1 -> select("Select COUNT(*) as countval from Customers where customerId = 1 ", ResultCount);
    table dt = check temp;
    while (dt.hasNext()) {
...
function testXAransactonFailed1 () returns (int, int) {
    ...
    int count2;
    //check whether update action is performed
    var temp = testDB1 -> select("Select COUNT(*) as countval from Customers where customerId = 2", ResultCount);
    table dt = check temp;
    while (dt.hasNext()) {
        ResultCount rs = check <ResultCount>dt.getNext();
        count1 = rs.COUNTVAL;
