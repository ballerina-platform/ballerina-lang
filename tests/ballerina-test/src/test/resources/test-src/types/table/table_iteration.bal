...
function testForEachInTableWithStmt() returns (int, int, float, string) {
    ...
    var temp = testDB->select("SELECT * from Person where id = 1", Person, false);
...
function testCountInTable() returns (int) {
    ...
    var temp = testDB->select("SELECT * from Person where id < 10", Person, false);
    table<Person> dt = check temp;
    int count = dt.count();
    _ = testDB->close();
    return count;
...
function testFilterTable() returns (int, int, int) {
    ...
    var temp = testDB->select("SELECT * from Person", Person, false);
    table<Person> dt = check temp;
...
function testMapTable() returns (string[]) {
    ...
    var temp = testDB->select("SELECT * from Person order by id", Person, false);
    table<Person> dt = check temp;
