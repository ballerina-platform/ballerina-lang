...
public class RetrySampleTestCase extends IntegrationTestCase {
    ...
    public void testMultiPart() throws IOException {
        ...
        String multipartBody = "--" + multipartDataBoundary + "\r\n" +
                ...
                "Content-Disposition: form-; name=\"filepart\"; filename=\"file-01.txt\"" + "\r\n" +
                ...
                "--" + multipartDataBoundary + "--" + "\r\n";
        Map<String, String> headers = new HashMap<>();
        headers.put(HttpHeaderNames.CONTENT_TYPE.toString(), "multipart/form-; boundary=" + multipartDataBoundary);
        HttpResponse response = HttpClientRequest.doPost(ballerinaServer
                        .getServiceURLHttp("retry")
                , multipartBody, headers);
        Assert.assertEquals(response.getResponseCode(), 200, "Response code mismatched");
        Assert.assertTrue(response.getHeaders().get(HttpHeaderNames.CONTENT_TYPE.toString())
                        .contains("multipart/form-;boundary=" + multipartDataBoundary),
                "Response is not form of multipart");
        Assert.assertTrue(response.getData().contains("form-;name=\"foo\"content-id: 0Part1")
                , "Message content mismatched");
        Assert.assertTrue(response.getData().
                        contains("form-;name=\"filepart\";filename=\"file-01.txt\"content-id: 1Part2")
                , "Message content mismatched");
    }
