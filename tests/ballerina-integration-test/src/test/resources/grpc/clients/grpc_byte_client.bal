...
function testLargeByteArray(string filePath) returns (string) {
    byteServiceBlockingClient blockingEp  = new ("http://localhost:8557");
    io:ReadableByteChannel rch = unsafeMarkUntainted(io:openReadableFile(filePath));
...
function unsafeMarkUntainted(io:ReadableByteChannel chan) returns @untainted io:ReadableByteChannel {
    return chan;
}