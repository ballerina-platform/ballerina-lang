...
public type HelloWorldBlockingClient client object {
    ...
    function __init(grpc:ClientEndpointConfig config) {
        ...
        io:println("|", descriptorMap100, "|");
        error? result = c.initStub("blocking", DESCRIPTOR_KEY, descriptorMap100);
        if (result is error) {
            panic result;
        } else {
            self.grpcClient = c;
...
public type helloWorldClient client object {
    ...
    function __init(grpc:ClientEndpointConfig config) {
        ...
        grpc:Client c = new;
        c.init(config);
        error? result = c.initStub("non-blocking", DESCRIPTOR_KEY, descriptorMap100);
        if (result is error) {
            panic result;
        } else {
            self.grpcClient = c;
