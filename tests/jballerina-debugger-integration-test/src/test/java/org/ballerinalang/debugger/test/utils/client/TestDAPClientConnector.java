...
public class TestDAPClientConnector {
    ...
    private TestStreamConnectionProvider createConnectionProvider(String balHome) {
        ...
        if (Utils.getOSName().toLowerCase(Locale.ENGLISH).contains("windows")) {
            ...
            processArgs.add("/c");
            processArgs.add(balHome + File.separator + "bin" + File.separator +
                    Constant.JBALLERINA_SERVER_SCRIPT_NAME + ".bat");
        } else {
            processArgs.add("bash");
            processArgs.add(balHome + File.separator + "bin/" + Constant.JBALLERINA_SERVER_SCRIPT_NAME);
        }
