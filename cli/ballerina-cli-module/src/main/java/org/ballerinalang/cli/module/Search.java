...
public class Search {
    ...
    private static void handleResponse(HttpURLConnection conn, int statusCode, String terminalWidth) {
        try {
            ...
            if (statusCode == HttpURLConnection.HTTP_OK) {
                ...
                if (payload.getIntValue(BStringValues.fromString("count")) > 0) {
                    ArrayValue modules = payload.getArrayValue(BStringValues.fromString("modules"));
                ...
                throw ErrorUtil.createCommandException(
                        payload.getStringValue(BStringValues.fromString("message")).getValue());
            }
        } finally {
            conn.disconnect();
        }
    ...
    private static void printModule(MapValue module, int dateColWidth, int versionColWidth, int authorsColWidth,
        ...
        String orgName = module.getStringValue(BStringValues.fromString("orgName")).getValue();
        String packageName = module.getStringValue(BStringValues.fromString("name")).getValue();
        printInCLI("|" + orgName + "/" + packageName, nameColWidth);

        String summary = module.getStringValue(BStringValues.fromString("summary")).getValue();

        if (descColWidth >= minDescColWidth) {
            printInCLI(summary, descColWidth - authorsColWidth);
            String authors = "";
            ArrayValue authorsArr = module.getArrayValue(BStringValues.fromString("authors"));
        ...
        long createTimeJson = module.getIntValue(BStringValues.fromString("createdDate"));
        printInCLI(getDateCreated(createTimeJson), dateColWidth);

        String packageVersion = module.getStringValue(BStringValues.fromString("version")).getValue();
