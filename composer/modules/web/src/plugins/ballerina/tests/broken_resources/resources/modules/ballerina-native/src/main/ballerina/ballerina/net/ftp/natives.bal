...
connector ClientConnector () {
    ...
    native action readFile (ClientConnector c, files:File file) (blob);
    ...
    native action copyFile (ClientConnector c, files:File source, files:File destination);
    ...
    native action moveFile (ClientConnector c, files:File target, files:File destination);

    @doc:Description { value:"Deletes a file from a given location"}
    @doc:Param { value:"c: FTP connector" }
    @doc:Param { value:"target: File/Directory that should be deleted" }
    native action deleteFile (ClientConnector c, files:File target);
    ...
    native action writeFile (ClientConnector c, blob content, files:File file);
