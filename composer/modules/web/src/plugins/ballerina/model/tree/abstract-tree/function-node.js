...
class AbstractFunctionNode extends Node {
    ...
    setReceiver(newValue, silent, title) {
        const oldValue = this.receiver;
        title = (_.isNil(title)) ? `Modify ${this.kind}` : title;
        this.receiver = newValue;

        this.receiver.parent = this;
    ...
    setInterfaceFunction(newValue, silent, title) {
        const oldValue = this.interfaceFunction;
        title = (_.isNil(title)) ? `Modify ${this.kind}` : title;
        this.interfaceFunction = newValue;
        if (!silent) {
            this.trigger('tree-modified', {
                origin: this,
                type: 'modify-node',
                title,
                data: {
                    attributeName: 'interfaceFunction',
                    newValue,
                    oldValue,
                },
            });
