...
public function main() returns error? {
    ...
    string dirPath = check filepath:build("foo", "bar");
    ...
    if (fileInfoResults is file:FileInfo) {
        ...
        io:println("Modified at ", fileInfoResults.getLastModifiedTime().
            toString());
    }
    ...
    string filePath = check filepath:build("foo", "bar", "bar.txt");
    ...
    string newFilePath = check filepath:build("foo", "bar1.txt");
    ...
    if (removeResults is ()) {
        io:println("Removed file at ", newFilePath);
    }
