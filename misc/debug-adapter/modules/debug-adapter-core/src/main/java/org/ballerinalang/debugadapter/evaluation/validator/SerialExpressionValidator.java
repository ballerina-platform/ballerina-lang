...
/**
 * Can be be used to validate debug expression evaluator user inputs and get the parsed syntax node for the expression,
 * if present.
...
public class SerialExpressionValidator extends Validator {
    ...
    public ExpressionNode validateAndGetResult(String source) throws Exception {
        ...
        // Retrieves the expression validator instance.
        ExpressionValidator exprValidator = (ExpressionValidator) validators.get(validators.size() - 1);
        // Retrieves the cached expression syntax tree, which was cached during the final validation phase.
        SyntaxTree cachedTree = exprValidator.getDebugParser().getSyntaxTreeFor(source);
        return exprValidator.getExpressionNodeFrom(cachedTree);
    }

    @Override
    public void validate(String source) throws Exception {
        loadValidators();
        // Runs the user input through out all validation phases.
        // Returns will return immediately if any validation/parsing error is detected.
    ...
    private void loadValidators() {
        ...
        validators.add(new InvalidInputValidator(debugParser));
        validators.add(new TopLevelDeclarationValidator(debugParser));
        validators.add(new StatementValidator());
