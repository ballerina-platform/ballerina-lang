...
/**
 * Code analyzer to generate the Testerina runtime function.
 */
public class MainFlowGenerationTask implements AnalysisTask<CompilationAnalysisContext> {

    private static final String TEST_EXEC_BAL = "test_execute.bal";
    ...
    private static final String TEST_EXEC_FUNCTION = "main";
    ...
    public void perform(CompilationAnalysisContext compilationAnalysisContext) {
        ...
        for (ModuleId moduleId : compilationAnalysisContext.currentPackage().moduleIds()) {
            ...
            DocumentConfig documentConfig = generateDocument(
                    compilationAnalysisContext.currentPackage().project(), module);
            module.modify().addTestDocument(documentConfig).apply();
        }
    }

    private static DocumentConfig generateDocument(Project project, Module module) {
        ...
        Path filePath = module.isDefaultModule()
                ? project.sourceRoot().resolve(TEST_DIRECTORY).resolve(TEST_EXEC_BAL)
                : project.sourceRoot().resolve("modules").resolve(module.moduleName().moduleNamePart())
                .resolve(TEST_DIRECTORY).resolve(TEST_EXEC_BAL);
        DocumentId newDocumentId = DocumentId.create(filePath.toString(), module.moduleId());
        return DocumentConfig.from(newDocumentId, newFileContent, TEST_DIRECTORY + TEST_EXEC_BAL);
    }
