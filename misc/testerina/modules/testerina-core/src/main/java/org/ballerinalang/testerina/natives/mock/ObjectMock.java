...
public class ObjectMock {
    ...
    public static BObject mock(BTypedesc bTypedesc, ObjectValue objectValue) {
        ObjectType objectValueType = (ObjectType) TypeUtils.getRepresentedType(objectValue.getType());
    ...
    public static BError validateFunctionName(String functionName, BObject mockObject) {
        ...
        if (!validateFunctionName(functionName,
                ((ObjectType) TypeUtils.getRepresentedType(genericMock.getType())).getMethods())) {
    ...
    public static BError validateFieldName(String fieldName, BObject mockObject) {
        ...
        if (!validateFieldName(fieldName,
                ((ObjectType) TypeUtils.getRepresentedType(genericMock.getType())).getFields())) {
        ...
        for (MethodType attachedFunction :
            ...
            if (attachedFunction.getName().equals(functionName)) {
                ...
                for (BIterator it = argsList.getIterator(); it.hasNext(); i++) {
                    Type paramType = TypeUtils.getRepresentedType(attachedFunction.getType().getParameters()[i].type);
    ...
    public static BError thenReturn(BObject caseObj) {
        ...
        ObjectType objectType = (ObjectType) TypeUtils.getRepresentedType(genericMock.getType());
    ...
    private static boolean validateUnionValue(Object returnVal, UnionType functionReturnType) {
        ...
        for (Type memberType : memberTypes) {
            if (TypeUtils.getRepresentedType(memberType).getTag() == TypeTags.PARAMETERIZED_TYPE_TAG) {
    ...
    private static boolean validateStreamValue(Object returnVal, StreamType streamType) {
        Type sourceType = TypeUtils.getRepresentedType(getType(returnVal));
        if (sourceType.getTag() == TypeTags.STREAM_TAG) {
            Type targetConstrainedType = TypeUtils.getRepresentedType(streamType.getConstrainedType());
            Type targetCompletionType = TypeUtils.getRepresentedType(streamType.getCompletionType());
    ...
    private static boolean validateReturnValue(
        ...
        for (MethodType attachedFunction : attachedFunctions) {
            if (attachedFunction.getName().equals(functionName)) {
                Type functionReturnType = TypeUtils.getRepresentedType(
                ...
                if (parameters.length != attachedFunction.getParameters().length) {
                    ...
                    for (int i = 0; i < parameters.length; i++) {
                        Type paramTypeAttachedFunc =
                                TypeUtils.getRepresentedType(attachedFunction.getType().getParameters()[i].type);
                        Type paramType = TypeUtils.getRepresentedType(parameters[i].type);
