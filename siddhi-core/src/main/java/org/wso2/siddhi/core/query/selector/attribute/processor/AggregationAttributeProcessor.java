/*
 * Copyright (c) 2005 - 2014, WSO2 Inc. (http://www.wso2.org)
 * All Rights Reserved.
 *
 * WSO2 Inc. licenses this file to you under the Apache License,
 * Version 2.0 (the "License"); you may not use this file except
 * in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
 * KIND, either express or implied.  See the License for the
 * specific language governing permissions and limitations
 * under the License.
 */
package org.wso2.siddhi.core.query.selector.attribute.processor;

import org.wso2.siddhi.core.config.SiddhiContext;
import org.wso2.siddhi.core.event.ComplexMetaEvent;
import org.wso2.siddhi.core.event.stream.MetaStreamEvent;
import org.wso2.siddhi.core.event.stream.StreamEvent;
import org.wso2.siddhi.core.executor.ExpressionExecutor;
import org.wso2.siddhi.core.executor.VariableExpressionExecutor;
import org.wso2.siddhi.core.query.selector.attribute.factory.OutputAttributeAggregatorFactory;
import org.wso2.siddhi.core.query.selector.attribute.handler.OutputAttributeAggregator;
import org.wso2.siddhi.query.api.definition.Attribute;
import org.wso2.siddhi.query.api.definition.StreamDefinition;
import org.wso2.siddhi.query.api.expression.Expression;

import java.util.List;
import java.util.Map;

public class AggregationAttributeProcessor extends AbstractAggregationAttributeProcessor implements NonGroupingAttributeProcessor {

    private OutputAttributeAggregator outputAttributeAggregator;
    private int outputPosition;

    public AggregationAttributeProcessor(Expression[] expressions, Map<String, StreamDefinition> tempStreamDefinitionMap,
                                         ComplexMetaEvent metaEvent, List<VariableExpressionExecutor> variableExpressionExecutors,
                                         OutputAttributeAggregatorFactory outputAttributeAggregatorFactory, SiddhiContext siddhiContext) {
       super(expressions, tempStreamDefinitionMap,outputAttributeAggregatorFactory,metaEvent,variableExpressionExecutors,siddhiContext) ;
        this.outputAttributeAggregator = sampleOutputAttributeAggregator;
    }

    private AggregationAttributeProcessor(List<ExpressionExecutor> expressionExecutors,OutputAttributeAggregator outputAttributeAggregator,SiddhiContext siddhiContext) {
        super(expressionExecutors,outputAttributeAggregator,siddhiContext);
    }


    public synchronized Object process(StreamEvent event) {
        return process(event, outputAttributeAggregator);
    }

    public void setOutputPosition(int position) {
        this.outputPosition = position;
    }

    @Override
    public AttributeProcessor cloneProcessor() {
        AggregationAttributeProcessor clonedAttributeProcessor = new AggregationAttributeProcessor(expressionExecutors,outputAttributeAggregator,siddhiContext);
        clonedAttributeProcessor.outputAttributeAggregator = clonedAttributeProcessor.sampleOutputAttributeAggregator;
        clonedAttributeProcessor.setOutputPosition(this.getOutputPosition());
        return clonedAttributeProcessor;
    }

    public int getOutputPosition() {
        return outputPosition;
    }

    @Override
    public void init(List<ExpressionExecutor> attributeExpressionExecutors, SiddhiContext siddhiContext) {

    }

    @Override
    protected Object execute(Object[] data) {
        return null;
    }

    @Override
    protected Object execute(Object data) {
        return null;
    }

    @Override
    public void destroy() {

    }

    @Override
    public Attribute.Type getReturnType() {
        return outputAttributeAggregator.getReturnType();
    }
}
