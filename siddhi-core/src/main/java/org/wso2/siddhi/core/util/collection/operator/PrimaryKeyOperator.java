...
public class PrimaryKeyOperator implements Operator {
    ...
    public Finder cloneFinder(String key) {
        return new PrimaryKeyOperator(expressionExecutor.cloneExecutor(key), candidateEventPosition, indexPosition);
    }
    ...
    public StreamEvent find(StateEvent matchingEvent, Object candidateEvents, StreamEventCloner candidateEventCloner) {
        ...
        StreamEvent streamEvent = ((PrimaryKeyEventHolder) candidateEvents).get(matchingKey);
        if (streamEvent == null) {
            return null;
        } else {
