...
public class Window implements FindableProcessor, Snapshotable {
     ...
     */
    public void init(Map<String, Table> tableMap, Map<String, Window> eventWindowMap, LatencyTracker latencyTracker, String queryName) {
        ...
        WindowProcessor internalWindowProcessor = (WindowProcessor) SingleInputStreamParser.generateProcessor(windowDefinition.getWindow(), metaStreamEvent, new ArrayList<VariableExpressionExecutor>(), this.executionPlanContext, tableMap, false, outputExpectsExpiredEvents, queryName);
    ...
    public CompiledCondition compileCondition(Expression expression, MatchingMetaInfoHolder matchingMetaInfoHolder, ExecutionPlanContext executionPlanContext, List<VariableExpressionExecutor> variableExpressionExecutors, Map<String, Table> tableMap, String queryName) {
        if (this.internalWindowProcessor instanceof FindableProcessor) {
            return ((FindableProcessor) this.internalWindowProcessor).compileCondition(expression, matchingMetaInfoHolder, executionPlanContext, variableExpressionExecutors, tableMap, queryName);
        } else {
